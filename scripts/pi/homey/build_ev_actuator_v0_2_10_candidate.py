#!/usr/bin/env python3
"""Build a Homey Advanced Flow candidate for EV Actuator v0.2.10.

Baseline is the live v0.2.9 flow exported with:
  homey api flow get-advanced-flows --json > /tmp/homey-flows.json

This script performs no Homey calls. It only rewrites the two EV actuator
HomeyScript cards in the selected flow and writes a candidate JSON file.
"""
...
EXPECTED_BASELINE_MARKER = "EM2_EV_ACTUATOR_V0.2.9"

MAIN_CODE = r'''// EV Actuator v0.2.10 SEMANTIC-TOKEN + DEADLINE STALE-STATE DEGRADE
const VERSION='EM2_EV_ACTUATOR_V0.2.10',CHARGER_ID='4d0b6913-d940-474e-95d6-b43f194c4119',FRESH_MS=120000;
...
const deadlineOverrideEligible=pp?.deadlineGuardApplied===true&&pp?.deadlineTeslaConnected===true&&targetStatus==='NUMERIC_DEADLINE_TARGET'&&targetSource==='REMAINING_KWH_OVER_TIME_TO_DEADLINE'&&Number.isInteger(targetW)&&targetW>0&&Number.isInteger(requestedA)&&requestedA>=6&&requestedA<=16&&Number.isInteger(deadlineMaxA)&&deadlineMaxA>=6&&deadlineMaxA<=16&&requestedA<=deadlineMaxA&&deadlineRemainingKWh!==null&&deadlineRemainingKWh>0&&Number.isFinite(deadlineAtMs)&&deadlineAtMs>Date.now();
...
try{
  ...
  const controlFresh=intentFresh&&adapterFresh&&gateFresh;
  ...
  const staleStateDeadlineAllowed=!stateFresh&&deadlineOverrideEligible&&connectedState&&!explicitUnsafe;
  ...
  if(!controlFresh)return await failClosed('STALE_CONTROL_INPUT',{intentFresh,adapterFresh,gateFresh,stateFresh});
  if(!coreSafeFresh&&!staleStateDeadlineAllowed)return await failClosed(stateFresh?'CORE_CHARGER_STATE_UNSAFE':'STALE_STATE_NO_DEADLINE_OVERRIDE',{chargeState:cs,stateAgeMs:age(state?.sampledAt),deadlineOverrideEligible});
  if(!adapterSafe||!numericOK)return await failClosed('ADAPTER_CONTRACT_INVALID');
  if(gate?.finalStatus!=='PASS')return await failClosed('VALIDATION_GATE_NOT_PASS',{gateStatus:gate?.finalStatus});
  ...
  await report(staleStateDeadlineAllowed?'READY_SESSION_CONTROL_DEGRADED_DEADLINE_STATE':'READY_SESSION_CONTROL',{targetW,targetA:requestedA,controlRevision:cr,coreRevision:coreR,gateStatus:'PASS',live:true,physicalWritePerformed:false,degraded:staleStateDeadlineAllowed,degradedReason:staleStateDeadlineAllowed?'EASEE_STATE_STALE_DEADLINE_OVERRIDE':null,stateAgeMs:age(state?.sampledAt),deadlineAt:pp?.deadlineAt??null});
  return true;
}catch(e){return await failClosed('RUNTIME_EXCEPTION',{error:String(e?.message||e)});}
'''

SESSION_CODE = r'''// EV Actuator v0.2.10 POST-SESSION + DEADLINE STALE-STATE DEGRADE
const VERSION='EM2_EV_ACTUATOR_V0.2.10',CHARGER_ID='4d0b6913-d940-474e-95d6-b43f194c4119',FRESH_MS=120000;
const IDS={live:'8d47e98d-e4bc-4f47-8c02-c2aca7f7a978',status:'ea1f8a44-2f6c-490e-9b86-bae761886cf9',intent:'04b57041-dd7f-41f7-a00a-f023afb1ccee',adapter:'f2118322-d59d-4aa8-b478-234effc3983c',gate:'4c66836b-77ae-43b5-b8e0-b32af15b57bc',state:'8e1efbb0-7999-494c-9429-7d274afacd79'};
const parse=x=>{try{return JSON.parse(String(x??''));}catch{return null;}};
const num=x=>{if(x===null||x===undefined||x==='')return null;const n=Number(x);return Number.isFinite(n)?n:null;};
...
const [liveVar,statusVar,intentVar,adapterVar,gateVar,stateVar]=await Promise.all([Homey.logic.getVariable({id:IDS.live}),Homey.logic.getVariable({id:IDS.status}),Homey.logic.getVariable({id:IDS.intent}),Homey.logic.getVariable({id:IDS.adapter}),Homey.logic.getVariable({id:IDS.gate}),Homey.logic.getVariable({id:IDS.state})]);
const intent=parse(intentVar?.value),adapter=parse(adapterVar?.value),gate=parse(gateVar?.value),state=parse(stateVar?.value),ev=intent?.targets?.ev||{},pp=intent?.policyProjection||{};
const targetW=num(ev?.target_W),targetStatus=String(ev?.status??''),targetSource=String(ev?.source??''),requestedA=num(adapter?.command?.requested_A),cr=String(adapter?.controlRevision||''),gcr=String(gate?.controlRevision||'');
const deadlineAtMs=Date.parse(String(pp?.deadlineAt||'')),deadlineRemainingKWh=num(pp?.deadlineRemainingKWh),deadlineMaxA=num(pp?.deadlineMaxA);
const deadlineOverrideEligible=pp?.deadlineGuardApplied===true&&pp?.deadlineTeslaConnected===true&&targetStatus==='NUMERIC_DEADLINE_TARGET'&&targetSource==='REMAINING_KWH_OVER_TIME_TO_DEADLINE'&&Number.isInteger(targetW)&&targetW>0&&Number.isInteger(requestedA)&&requestedA>=6&&requestedA<=16&&Number.isInteger(deadlineMaxA)&&deadlineMaxA>=6&&deadlineMaxA<=16&&requestedA<=deadlineMaxA&&deadlineRemainingKWh!==null&&deadlineRemainingKWh>0&&Number.isFinite(deadlineAtMs)&&deadlineAtMs>Date.now();
...
const coreSafeFresh=stateFresh&&!['','unknown'].includes(cs)&&!explicitUnsafe;
const staleStateDeadlineAllowed=!stateFresh&&deadlineOverrideEligible&&connectedState&&!explicitUnsafe;
const controlCoherent=liveVar.value===true&&intentFresh&&adapterFresh&&adapter?.valid===true&&gate?.finalStatus==='PASS'&&!!cr&&gcr===cr&&gate?.adapterControlRevision===cr&&gateFresh;
const coherent=controlCoherent&&(coreSafeFresh||staleStateDeadlineAllowed);
...
if(!coherent||requestedA===0){if(currentA!==0)await charger.setCapabilityValue('target_charger_current',0);await report('POST_SESSION_ABORT_ZERO',{requestedA,previousA:currentA,controlRevision:cr,physicalWritePerformed:currentA!==0,controlCoherent,stateFresh,deadlineOverrideEligible,chargeState:cs});return true;}
...
await report(staleStateDeadlineAllowed?(currentA===requestedA?'NOOP_DEGRADED_DEADLINE_STATE':'WRITE_OK_DEGRADED_DEADLINE_STATE'):(currentA===requestedA?'NOOP_POST_SESSION_TARGET':'WRITE_OK_POST_SESSION'),{targetA:requestedA,previousA:currentA,controlRevision:cr,coreRevision:num(state?.revision),physicalWritePerformed:currentA!==requestedA,degraded:staleStateDeadlineAllowed,degradedReason:staleStateDeadlineAllowed?'EASEE_STATE_STALE_DEADLINE_OVERRIDE':null,stateAgeMs:age(state?.sampledAt),deadlineAt:pp?.deadlineAt??null});
return true;
'''
...
def main() -> int:
    ...
    ap.add_argument("--input", default="/tmp/homey-flows.json")
    ap.add_argument("--output", default="/tmp/ev-actuator-v0.2.10-candidate.json")
    ap.add_argument("--flow-id", default=FLOW_ID_DEFAULT)
    args = ap.parse_args()

    src = Path(args.input)
    data = json.loads(src.read_text(encoding="utf-8"))
    if args.flow_id not in data:
        raise SystemExit(f"flow not found: {args.flow_id}")

    flow = data[args.flow_id]
    ...
    if EXPECTED_BASELINE_MARKER not in old_main or EXPECTED_BASELINE_MARKER not in old_session:
        raise SystemExit("refusing: expected live v0.2.9 baseline marker not present in both cards")

    cards[MAIN_CARD_ID]["args"]["code"] = MAIN_CODE
    cards[SESSION_CARD_ID]["args"]["code"] = SESSION_CODE
    flow["name"] = "EM v2 | 60 Actuator | EV Power v0.2.10 SEMANTIC-TOKEN LIVE + EASEE SESSION"
    ...
    print(f"main_card={MAIN_CARD_ID} -> v0.2.10")
    print(f"session_card={SESSION_CARD_ID} -> v0.2.10")
    print("homey_calls=0")
    return 0
