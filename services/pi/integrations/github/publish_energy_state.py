...
def load_source() -> dict:
    ...
    if meta.get("schema_version") != EXPECTED_SCHEMA_VERSION:
        raise SystemExit(f"FAIL: schema mismatch: {meta.get('schema_version')!r} != {EXPECTED_SCHEMA_VERSION!r}")
    ...
    if age > MAX_SOURCE_SAMPLE_AGE_SECONDS:
        raise SystemExit(f"FAIL: stale source_sample_at: age={age:.1f}s max={MAX_SOURCE_SAMPLE_AGE_SECONDS}s")
    return payload
...
def main() -> None:
    ...
    parser.add_argument("--dry-run", action="store_true", help="Validate and render publication metadata without contacting GitHub")
    ...
    body = {"message": "chore: publish Pi energy state telemetry", "content": content, "branch": BRANCH}
    if sha:
        body["sha"] = sha
