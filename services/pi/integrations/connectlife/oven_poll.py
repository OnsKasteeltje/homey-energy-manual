...
def parse_args():
    ...
    parser.add_argument("--watch", action="store_true", help="Keep polling with the same authenticated API session.")
    parser.add_argument("--interval", type=int, default=60, help="Polling interval in seconds when --watch is used (default: 60).")
    parser.add_argument("--max-failures", type=int, default=3, help="Exit after this many consecutive failed polls so systemd can restart the service (default: 3).")
    parser.add_argument("--output", type=Path, help="Atomically write the latest EMS oven state to this JSON file.")
...
async def run():
    ...
    while True:
        try:
            ...
            failures += 1
            print(f"ConnectLife poll mislukt ({failures}/{args.max_failures}): {exc}", file=sys.stderr, flush=True)
