...
// Pi planner component. P1 is authoritative. No device/control writes.
...
// Previous SHADOW state is used only for dwell/hysteresis.
...
export function decideEvPhaseShadow(input, previous={}, nowMs=Date.now(), cfg=CONFIG) {
  ...
  const actualShapeValid=
    actualA===0 ||
    actualPhaseCount===3 ||
    (actualPhaseCount===1 && (actualPhase===null || (actualPhase>=1 && actualPhase<=3)));
  ...
  // Observability only: Easee/Equalizer owns the physical phase used in 1P.
  // If actual 1P phase is known, add the running EV load back to that phase.
  const phaseAvailableW=phases.map((w,i)=>{
    const phaseNo=i+1;
    const addBackW=actualA*cfg.voltageV*(
      actualPhaseCount===3 ||
      (actualPhaseCount===1 && actualPhase===phaseNo) ? 1 : 0
    );
    return Math.max(0,-w+addBackW);
  });
  const bestPhaseAvailableW=Math.max(...phaseAvailableW);
  const bestPhaseObservation=phaseAvailableW.indexOf(bestPhaseAvailableW)+1;
  ...
  if(prevMode==='3P') {
    if(availableTotalW<cfg.leave3pW && modeDwellOK) {
      if(bestPhaseAvailableW>=cfg.start1pW){mode='1P';reason='3P_TO_1P_TOTAL_SURPLUS_LOW';}
    ...
    } else if(bestPhaseAvailableW<cfg.stop1pW && modeDwellOK) {
      mode='OFF';reason='1P_TO_OFF_PHASE_SURPLUS_LOW';
    }
  } else {
    if(availableTotalW>=cfg.enter3pW) {
      mode='3P';reason='OFF_TO_3P_TOTAL_SURPLUS';
    } else if(bestPhaseAvailableW>=cfg.start1pW) {
      mode='1P';reason='OFF_TO_1P_AVAILABLE_PHASE';
    }
  }

  let requestedA=0;
  if(mode==='3P') requestedA=clamp(Math.floor(availableTotalW/(3*cfg.voltageV)),cfg.minA,maxA);
  if(mode==='1P') requestedA=clamp(Math.floor(bestPhaseAvailableW/cfg.voltageV),cfg.minA,maxA);
...
function shadow(mode,requestedA,reason,input,previous,nowMs,cfg){
  ...
  return {
    ...
    preferredPhaseObservability:mode==='1P'?input?.bestPhaseObservation??null:null,
    reason,
    ...
    onePhaseDecisionBasis:'BEST_PHASE_SURPLUS_FOR_SIZING_ONLY',
