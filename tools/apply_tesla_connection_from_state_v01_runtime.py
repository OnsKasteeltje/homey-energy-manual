...
source_ts = (
    (payload.get('meta') or {}).get('source_sample_at')
    or (payload.get('meta') or {}).get('generated_at')
    or datetime.now(timezone.utc).isoformat().replace('+00:00','Z')
)
state_revision = (payload.get('meta') or {}).get('state_revision')
...
fetch_idx = next((i for i,l in enumerate(lines) if 'fetch_energy_state.py' in l), -1)
store_idx = next((i for i,l in enumerate(lines) if 'store_tesla_connection_from_state.py' in l), -1)
pv_idx = next((i for i,l in enumerate(lines) if 'fetch_pv_forecast.py' in l), -1)
