...
Status: **authoritative runtime reconciliation baseline for documentation generation**.
...
|---|---|---|---|
| LIVE | `EM v2 | 00 Core Tick | v0.11f (Planner Tesla Headroom)` | `227f8d3b-7551-46dd-837d-1b8c69add824` | enabled; 5-minute Core; no physical device writes; Planner Tesla admission uses projected grid `gridW + 4140 W <= 4000 W`; `PUB_VERSION=EM2_CORE_STATE_V0.11f`, schema 2.12 |
| ACTIVE SHADOW | `EM v2 | 12 Input | Core Snapshot Aggregator v0.2 SHADOW NO-QUOOKER` | `758f3353-51f5-4e68-a1f4-3acf30ec5a87` | active input aggregation / parity architecture |
...
| LIVE config | `EM v2 | 05 Config | EMS Settings Sync v0.4 TARGETED 15-MIN LOW-LOAD` | `9193b3ae-1e3d-4b52-aa95-60aff099e68a` | enabled; targeted Logic reads; 15-minute cadence |
...
| ACTIVE SHADOW | `EM v2 | 60 Adapter | EV Power v0.1.1 TARGETED-READ SHADOW` | `953e9b18-3576-4557-b940-ed4a64eb2516` | shadow adapter retained for architecture/validation |
...
The inventory still contains temporary candidates that require separate dependency review before deletion, including `TEMP | PBTH API vs Card A-B v0.3`, `TEMP | Planner v0.4.4 Smoke Shutdown`, multiple `TEMP | EM2 Control EV Observer` / `Observer Setup` flows, `TEMP | EM2 Control EV Observer Setup 2026-08-29`, and standard flow `TEMP | EM2 Control EV Semantic Observer 2026-08-29`. The cleanup executor/helper is temporary and must not be documented as architecture.

## Known reconciliation gap: exact Core v0.11f source

Direct Homey inspection on 2026-08-30 verified the live v0.11f Advanced Flow and its complete HomeyScript. The repository's previous Core README still described v0.11c/v0.11d and is stale. An automated one-shot attempt to snapshot the exact HomeyScript directly from Homey into `src/homey/core/core-v0.11f.live-homey.js` did not result in a GitHub file and therefore is **not considered successful evidence**.

Until `src/homey/core/core-v0.11f.live-homey.js` (or an equivalent exact full-runtime capture) exists and is verified against the live Advanced Flow, repository Markdown is **not yet certified fully code-synchronous for Core**. Do not generate a final authoritative software architecture document claiming full Homey↔GitHub code parity before closing this item.

## Certification state

- Flow inventory reconciliation: **PASS** for the observed 2026-08-30 inventory.
- Settings Sync v0.4 status/version: **PASS** from direct Homey validation performed earlier on 2026-08-30.
- Contract Price Adapter v0.10 and FIXED switch acceptance: **PASS** from controlled runtime validation performed earlier on 2026-08-30.
- Cleanup Round 2A inventory: **PASS** after post-cleanup list verification.
- Exact Core v0.11f source in GitHub: **OPEN / BLOCKING**.
- Overall Homey ↔ GitHub documentation-generation certification: **NOT YET PASS** until exact Core v0.11f runtime source is captured and repository architecture Markdown is then rechecked against this registry.
