...
As of 2026-09-26, production still runs EV phase writer v0.4.1 while v0.4.2 is prepared for guarded cutover. Live 3P→1P validation confirmed the physical phase change itself can complete correctly, but v0.4.1 may keep the session paused through a long multi-stage transition and its persisted actuator status can lag the already-resumed physical charger state. V0.4.2 therefore adds active electrical phase proof plus Easee observation 38 confirmation for true phase changes, while retaining the existing fail-closed transition sequence. The one-time v0.4.2 upgrade is only permitted from a quiescent v0.4.1 state: stable actuator/transition status, aligned phase, normal circuit cap, physically matching positive charge current, unchanged control/phase/target across a 10-second window, and a final immediate pre-push recheck. A failed post-deploy validation restores the exact previous Advanced Flow; the upgrader must not raise an unknown temporary circuit cap merely to force progress.
