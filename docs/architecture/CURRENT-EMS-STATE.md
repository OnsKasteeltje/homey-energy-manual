...
Derived household history is owned by `services/pi/history/build_house_energy_history.py`. It creates `house_energy_intervals` from consecutive complete five-counter snapshots. Each interval retains P1 import/export and all three PV deltas separately, plus aggregate PV and `house_kwh = import_kwh + pv_total_kwh - export_kwh`. A counter decrease is stored as a discontinuity with null energy values, never as negative energy. Incomplete intermediate counter snapshots are skipped while retaining the last complete five-counter baseline, because cumulative counters permit safe bridging; the resulting interval is quality-marked `gap` when the complete-to-complete span exceeds 15 minutes. Gaps over 15 minutes therefore remain numerically derivable, while presentation/aggregation must surface that reduced coverage rather than silently treating it as complete.
