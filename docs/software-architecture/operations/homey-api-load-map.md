...
source:
  ...
  - Publisher v1.0.9 HARD-GATE runtime inspected and smoke-tested 2026-08-29
...
## Current throttling investigation baseline — 2026-08-29
...
| `EM v2 | 81 Observability | EV Control Status v0.1` | `f6edba38-ddf1-45e5-890e-c183aa2055d5` | OFF except controlled one-shot diagnostic | Gate/actuator change when enabled; manual Start remains | one `getVariables()`; one small publish-cache Logic write after successful publication; **0 device reads/writes** | steady-state 1 GitHub PUT per changed control status; GET only on cache initialization or conflict recovery | kept disabled outside controlled EV diagnostics during throttle investigation | LOW/MEDIUM | OBSERVABILITY |
...
| `EM v2 | 40 Data | Publisher v1.0.9 HARD-GATE LOW-LOAD` | `fe84bc17-72d4-4fbb-9a69-b3d751b0ffcd` | **ON** | `EM2_Public_State` changed + 2 s; manual Start remains | 5 targeted Logic reads per trigger; if `<15 min` since last publish: immediate return with **0 status/diagnostic writes** | **0 GitHub I/O inside hard-gate window**; when due: GET + PUT, with retry GET only on 409/422 | hard minimum 15 min between actual publishes; max nominal external publication about 4/hour; A/B candidate after throttling disappeared while Publisher was OFF | LOW on gated runs / MEDIUM when publishing | PRODUCTION OUTPUT / INVESTIGATION |
...
EV Control Status is downstream-only observability. It performs one Logic enumeration per event and no device access. Because exact revision coherence is safety evidence for a positive LIVE smoke, a new natural Gate revision is allowed to produce one external status publication when the observability flow is deliberately enabled. The cached GitHub SHA keeps steady state to one PUT; GET is limited to cache initialization or conflict recovery. During the throttling A/B baseline it remains OFF except for controlled one-shot diagnostics.

### Publisher hard-gate path

```text
Core Tick (5 min)
  -> EM2_Public_State changes
      -> Publisher v1.0.9 (+2 s)
          -> 5 targeted Logic reads
          -> age since last publish < 15 min: STOP
          -> age >= 15 min and revision/heartbeat due: GitHub GET + PUT
```

The trigger itself can still occur on each Public State change, but the expensive branch is bounded. An early-gated run performs no GitHub I/O and no publisher status/diagnostic writes. This separates trigger frequency from external publication frequency and is the active A/B mitigation for the 2026-08-29 throttling correlation.
...
- On 2026-08-29 Homey became reachable again after the Data Publisher was disabled. This is a **strong correlation, not yet proof of sole causality**.
- Publisher v1.0.9 HARD-GATE LOW-LOAD was enabled on 2026-08-29 as the controlled A/B mitigation. Its manual smoke returned `Successfully started the Flow.` without throttling. Actual GitHub publication is now hard-limited to at most once per 15 minutes; early-gated triggers do only five targeted Logic reads and no status/diagnostic writes.
- Watchdog, Immutable Day Archive and washer/dryer analysis/logging/publication remain disabled during the clean throttling isolation.
...
- EV Control Status v0.1 was added on 2026-08-28 to replace temporary runtime probes with permanent downstream observability. It publishes revision, `EV_target_W`, `requested_A`, Gate status/revisions, actuator status and LIVE state to `docs/data/ev-control-status.json` without device access. It is currently kept OFF outside controlled one-shot diagnostics.
...
- The positive Tesla LIVE regulation smoke has **not yet been performed** after this safety hardening. Runtime proof still requires a fresh natural positive EV target, expected adapter current, current Gate PASS/coherence and a controlled LIVE smoke.
...
7. Keep Publisher v1.0.9 enabled during the A/B window and judge it on sustained multi-cycle reachability; do not weaken the 15-minute hard gate merely to increase website freshness.
8. Execute the controlled positive EV LIVE smoke only when `docs/data/ev-control-status.json` shows a fresh positive `targetW`, the expected `requestedA`, Gate `PASS` and `coherent=true`; do not weaken safety criteria to make the smoke pass.
