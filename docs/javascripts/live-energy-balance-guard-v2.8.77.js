(function(){
  ...
  function purgeLegacySourceSkew(root){
    if(!root)return;
    root.querySelectorAll('.energy-balance-warning').forEach(el=>{
      const txt=String(el.textContent||'').toLowerCase();
      if(txt.includes('balans indicatief')||txt.includes('bronmomenten verschillen')) el.remove();
    });
    const top=root.querySelector('.energy-topline');
    if(top){
      [...top.querySelectorAll('span,strong,div,p')].forEach(el=>{
        const txt=String(el.textContent||'').trim().toLowerCase();
        if((txt.includes('balans indicatief')||txt.includes('bronmomenten verschillen')) && !el.querySelector('*')) el.remove();
      });
    }
  }
  ...
  function apply(){
    ...
    try{
      ...
      // Measured-first policy: direct P1/PV/battery measurements are authoritative.
      // SOURCE_SKEW does not downgrade an otherwise physically valid balance.
      ...
      root.dataset.balanceGuardVersion='2.8.77';
  ...
  function reconcile(){ensureAttached();schedule();[80,250,600,1200,2500].forEach(ms=>setTimeout(()=>{ensureAttached();apply();},ms));}
  ...
  documentObserver=new MutationObserver(()=>{const root=document.getElementById('live-energy-flow');if(root!==observedRoot)reconcile();else purgeLegacySourceSkew(root);});
