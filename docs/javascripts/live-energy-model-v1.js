(function(){
  ...
  function buildViewModel(raw,freshness){
    ...
    const energyBudgetRaw=raw.energy_budget||{};
    ...
    const house=Number.isFinite(Number(energyBudgetRaw.house_load_w))
      ? pos(energyBudgetRaw.house_load_w)
      : Math.max(0,pv+grid+discharge-charge);
    ...
    const otherKnownExtras=['dishwasher','quooker'].reduce((sum,key)=>{
      ...
      const known=load&&load.power_w!==null&&load.power_w!==undefined&&Number.isFinite(Number(load.power_w));
      return sum+(known?pos(load.power_w):0);
    },0);
