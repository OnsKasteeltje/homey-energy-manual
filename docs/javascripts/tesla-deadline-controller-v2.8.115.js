(function(){
  ...
  window.HomeEnergyFrontend.teslaDeadlineController='2.8.115';
  ...
  function coreTerminal(t=coreTesla){
    if(!t||typeof t.deadline_active!=='boolean'||t.deadline_active===true)return false;
  ...
  function reconcilePending(){
    if(coreTerminal()&&pendingCommand?.active===true){
      const same=normalizeDeadline(pendingCommand.deadline||'')===normalizeDeadline(coreTesla?.deadline_at||'');
      if(same||deadlinePast(pendingCommand.deadline)){pendingCommand=null;pendingFailure='';}
    }
  ...
  function clearStaleDraft(){
    if(!coreTerminal()||!draft?.active)return;
    const same=normalizeDeadline(draft.deadline||'')===normalizeDeadline(coreTesla?.deadline_at||'');
    if(same||deadlinePast(draft.deadline))draft=null;
  }
  ...
  async function loadData(){
    ...
    coreTesla=core?.tesla||coreTesla;config=cfg;command=cmd;reconcilePending();clearStaleDraft();
  }
  function effectiveCommand(){return pendingCommand||command;}
  function activeState(){
    if(coreTerminal())return false;
  ...
  function ensureDefaults(wrap){
    const deadline=wrap?.querySelector('.tesla-inline-deadline');if(deadline){deadline.min=nowLocalMinute();if(!deadline.value||deadlinePast(deadline.value))deadline.value=`${today()}T20:00`;}
    const amps=wrap?.querySelector('.tesla-inline-amps');if(amps&&!amps.value)amps.value='11';
  }
  function bind(wrap){
    if(!wrap||wrap.dataset.bound==='2.8.115')return;
    wrap.dataset.bound='2.8.115';
    ...
    wrap.querySelectorAll('input[name="tesla-native-mode"]').forEach(r=>r.addEventListener('change',()=>{
      ...
      if(active){
        ensureDefaults(wrap);
        if(coreTerminal()){
          const current=wrap.querySelector('.tesla-inline-current'),target=wrap.querySelector('.tesla-inline-target');
          if(current)current.value='';if(target)target.value='';
        }
      }
      rememberDraft(wrap);
    }));
  ...
  function populate(){
    const wrap=native();if(!wrap)return false;bind(wrap);reconcilePending();clearStaleDraft();
    if(!applyDraft(wrap)){
      ...
      const hasSoc=active&&Number.isFinite(currentSoc)&&Number.isFinite(targetSoc)&&targetSoc>currentSoc;
      ...
      const deadline=wrap.querySelector('.tesla-inline-deadline');if(deadline){deadline.min=nowLocalMinute();deadline.value=active?remoteDeadline():`${today()}T20:00`;}
    ...
    wrap.dataset.coreDeadlineActive=String(Boolean(coreTesla?.deadline_active));
    wrap.dataset.coreDeadlineStatus=String(coreTesla?.deadline_status||'');
    return true;
  }
  async function refresh(){if(loading)return;loading=true;try{await loadData();if(!populate())scheduleBootReady();}finally{loading=false;}}
  function scheduleBootReady(){
    if(native()){clearTimeout(bootTimer);bootTimer=null;bootAttempts=0;if(config||command||coreTesla||pendingCommand)populate();else refresh();return;}
    if(bootAttempts>=40)return;bootAttempts+=1;clearTimeout(bootTimer);bootTimer=setTimeout(scheduleBootReady,125);
  }
  async function save(wrap){
    ...
    try{
      ...
      command=j.command||command;pendingCommand=j.command||{active:d.active,deadline:d.active?d.deadline:'',currentSoc:d.active?currentSoc:null,targetSoc:d.active?targetSoc:null,maxA,requestedAt:new Date().toISOString()};
  ...
  document.addEventListener('energycorev2state',e=>{coreTesla=e.detail?.raw?.tesla||coreTesla;reconcilePending();clearStaleDraft();setTimeout(populate,0);});
