(function(){
  ...
  function setPanel(root,mode,house,grid,measuredAssigned,estimatedAssigned,other){
    ...
    if(mode==='valid'){
      ...
      if(strong)strong.textContent=`P1 ${fmtGrid(grid)}`;
      if(span)span.textContent='P1 actueel · P1/PV niet tijdgelijk · afgeleide Huis/Overig-balans tijdelijk niet beschikbaar · geen control-impact';
    }else{
  ...
  function apply(){
    ...
    try{
      ...
      if(physicalValid){
        ...
        setNode(houseNode,fmtGrid(grid),'P1 actueel · huisbalans tijdelijk niet beschikbaar');
      ...
      root.dataset.balanceMeasurementPolicy=mode==='async'?'p1-source-visible-derived-balance-gated':'core-derived-gate-authoritative';
      root.dataset.balanceControlImpact='none';
      root.dataset.balanceGuardVersion='2.8.126';
    }finally{applying=false;}
  }

  // Core determines whether derived House/Overig is trustworthy. When that derived gate is
  // closed because P1/PV are asynchronous, keep the authoritative P1 source measurement visible
  // but never reconstruct or resurrect House/Overig from non-simultaneous source samples.
