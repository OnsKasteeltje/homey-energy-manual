(function(){
  ...
  window.HomeEnergyFrontend.teslaDeadlineController='2.8.106';
  ...
  function activeState(){
    // The command is the configuration source of truth for the edit form.
    // Runtime snapshots can lag several seconds and must not roll the form back.
  ...
  function remoteDeadline(){
    // Same rule as activeState: prefer the latest saved command for editable config.
    return normalizeDeadline(command?.deadline||snapshot?.evDeadlineTime||snapshot?.EVDeadlineTime||'');
  }

  function bind(wrap){
    if(!wrap||wrap.dataset.bound==='2.8.106')return;
    wrap.dataset.bound='2.8.106';
