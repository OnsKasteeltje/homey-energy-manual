(function(){
  ...
  function renderRaw(r,freshness){
    ...
    // Laag 1: productie
    svg+=node(40,25,380,110,'PRODUCTIE','SolarEdge SE3680H',fmt(se),'PV-omvormer','source');
    svg+=node(510,25,380,110,'PRODUCTIE','GoodWe GW4200D-NS',fmt(gw42),'PV-omvormer','source');
    svg+=node(980,25,380,110,'PRODUCTIE','GoodWe GW2000-XS',fmt(gw20),'PV-omvormer','source');
    svg+=path('M230 135 V180 H700 V245',se,'pv',se>0);
    svg+=path('M700 135 V245',gw42,'pv',gw42>0);
    svg+=path('M1170 135 V180 H700 V245',gw20,'pv',gw20>0);

    // Laag 2: grid - huis - batterij
    svg+=node(80,270,330,125,'NET / METER (P1)','Grid',fmt(Math.abs(grid)),grid>0?'import':grid<0?'export':'in balans','grid');
    svg+=node(535,270,330,125,'WONING','Huis',fmt(house),'centrale energiebalans','house');
    svg+=node(990,270,330,125,'BATTERIJ','Victron batterij',charge?`Laden ${fmt(charge)}`:discharge?`Ontladen ${fmt(discharge)}`:'Niet actief',charge||discharge?'live energiestroom':'voorbereid op ESS','battery');
    if(importW>0)svg+=path('M410 332 H535',importW,'grid',true,`Grid → Huis · ${fmt(importW)}`,472,316);else if(exportW>0)svg+=path('M535 332 H410',exportW,'grid',true,`Huis → Grid · ${fmt(exportW)}`,472,316);else svg+=path('M410 332 H535',0,'grid',false,'in balans',472,316);
    if(charge>0)svg+=path('M865 332 H990',charge,'battery',true,`Huis → Accu · ${fmt(charge)}`,927,316);else if(discharge>0)svg+=path('M990 332 H865',discharge,'battery',true,`Accu → Huis · ${fmt(discharge)}`,927,316);else svg+=path('M865 332 H990',0,'battery',false,'inactief',927,316);

    // Laag 3: hoofdverbruikers
    const main=[
      {x:25,title:'Tesla',value:fmt(tesla),sub:t.charging?'laden':(t.connected?'aangesloten':'niet aangesloten'),w:tesla,active:tesla>0,sx:610},
      {x:305,title:'Boiler',value:fmt(boiler),sub:hw.boiler_on?'aan':'uit',w:boiler,active:boiler>0,sx:655},
      {x:585,title:'Ruimteverwarming',value:fmt(quatt),sub:heatSub,w:quatt,active:quatt>0,sx:700},
      {x:865,title:'Wasmachine',value:washer.value,sub:washer.sub,w:washer.power,active:washer.known&&washer.power>0,sx:745},
      {x:1145,title:'Droger',value:dryer.value,sub:dryer.sub,w:dryer.power,active:dryer.known&&dryer.power>0,sx:790}
    ];
    main.forEach(c=>{const cx=c.x+115;svg+=path(`M${c.sx} 395 V485 H${cx} V545`,c.w,'grid',c.active);svg+=node(c.x,545,230,115,'HOOFDVERBRUIKER',c.title,c.value,c.sub,'load');});

    // Laag 4: Overig als aparte restcategorie onder de hoofdverbruikers
    svg+=path('M700 395 V710 H700 V770',other,'grid',other>0);
    svg+=node(520,770,360,125,'RESTVERBRUIK','Overig',fmt(other),otherSub,'load residual');
    svg+=`<text x="700" y="940" text-anchor="middle" class="energy-rule">Overig is een zelfstandige restcategorie: woning minus alle betrouwbaar afzonderlijk bekende vermogens.</text></svg>`;
