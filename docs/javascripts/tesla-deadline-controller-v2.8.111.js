(function(){
  ...
  window.HomeEnergyFrontend.teslaDeadlineController='2.8.111';

  let config=null,command=null,coreTesla=null,loading=false,draft=null;
  ...
  async function loadData(){
    ...
    coreTesla=core?.tesla||coreTesla;config=cfg;command=cmd;
  }
  ...
  function lifecycle(){
    ...
    const amps=Math.max(0,Number(coreTesla?.requested_a)||Number(command?.maxA)||0);
    ...
    if(deadlinePast(coreTesla?.deadline_at||command?.deadline)&&remaining>0.01)return {kind:'missed_open',status,remaining,amps};
    return {kind:'none',status,remaining,amps};
  }
  function remoteDeadline(){return normalizeDeadline(coreTesla?.deadline_at||command?.deadline||'');}
  ...
  function bind(wrap){
    if(!wrap||wrap.dataset.bound==='2.8.111')return;
    wrap.dataset.bound='2.8.111';
  ...
  function populate(){
    ...
    if(!applyDraft(wrap)){
      ...
      const maxA=Number(command?.maxA??coreTesla?.requested_a??11);
      const off=wrap.querySelector('input[name="tesla-native-mode"][value="off"]'),on=wrap.querySelector('input[name="tesla-native-mode"][value="on"]');
    ...
    if(msg&&!draft){const text=statusText(lifecycle());msg.textContent=text||(linked?'':'Write-route niet gekoppeld');msg.dataset.state=text?'info':'';}
    return true;
  }
  ...
  function scheduleBootReady(){
    if(native()){
      ...
      if(config||command||coreTesla)populate();else refresh();
      return;
    }
  ...
  document.addEventListener('energycorev2state',e=>{coreTesla=e.detail?.raw?.tesla||coreTesla;setTimeout(()=>{populate();},0);});
