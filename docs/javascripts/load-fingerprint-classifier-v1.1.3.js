(function(){
  ...
  // Direct AEG/Homey appliance state remains primary when usable.
  // Washer electrical recognition reconstructs gross L2 house load using the
  // validated phase map (L2 PV = GoodWe 4200) and removes directly measured
  // boiler power on L2. This keeps boiler overlap usable without inventing power.
  const VERSION='1.1.3';
  const HISTORY_KEY='em2-load-fingerprint-history-v3';
  ...
  function aegSourceQuality(load){
    ...
    if(stale)status='STALE';else if(inconsistent)status='INCONSISTENT';else if(usable&&directActive)status='HEALTHY_ACTIVE';else if(usable)status='HEALTHY_IDLE';
  ...
  function l2WasherSignal(raw){
    if(!finite(raw?.grid?.l2_w)||!finite(raw?.pv?.goodwe_4200_w))return null;
    const p1L2=Number(raw.grid.l2_w);
    const pvL2=Math.max(0,Number(raw.pv.goodwe_4200_w));
    const grossL2=p1L2+pvL2;
    const boilerPower=finite(raw?.hot_water?.boiler_power_w)?Math.max(0,Number(raw.hot_water.boiler_power_w)):0;
    const boilerActive=raw?.hot_water?.boiler_on===true&&boilerPower>ACTIVE_THRESHOLD_W;
    const signal=Math.max(0,grossL2-(boilerActive?boilerPower:0));
    return {signal_w:signal,gross_l2_w:grossL2,p1_l2_w:p1L2,pv_l2_w:pvL2,boiler_w:boilerActive?boilerPower:0,boiler_compensated:boilerActive};
  }
  ...
  function addHistory(raw){
    const ts=nowFromRaw(raw),l2=l2WasherSignal(raw);
    const row={
      ts,
      residual_w:residualPower(raw),
      l2_signal_w:l2?.signal_w??null,
      l2_gross_w:l2?.gross_l2_w??null,
      l2_pv_w:l2?.pv_l2_w??null,
      boiler_compensated:l2?.boiler_compensated===true,
      boiler_w:l2?.boiler_w??0,
    ...
    rows=rows.slice(-60);writeHistory(rows);return rows;
  }
  ...
  function cleanWasherRows(rows){
    // Boiler is deliberately NOT excluded: it is compensated using its direct L2 measurement.
    // Tesla remains excluded because its three-phase contribution is not reconstructed here.
    return rows.filter(x=>!x.tesla_active&&finite(x.l2_signal_w));
  }
  const bandOf=w=>w>=1800&&w<=2700?'HEAT':(w>=250&&w<=800?'WORK':(w>=40&&w<=260?'REST':null));
  ...
  function blindAnchor(recent){
    ...
    for(let i=0;i<=clean.length-5;i++){
      const window=clean.slice(i),bands=window.map(x=>bandOf(Number(x.l2_signal_w))).filter(Boolean);
      ...
      if(work>=2&&rest>=1&&alternations>=2&&spanMin>=10)return {ts:Number(window[0].ts),mode:'P1_SEQUENCE'};
    }
    return null;
  }
  ...
  function washerSequenceCandidate(raw,history){
    ...
    const cycle=recent.slice(anchor),clean=cleanWasherRows(cycle);if(clean.length<4)return null;
    const startTs=Number(cycle[0].ts),elapsedMin=(ts-startTs)/60000;
    const early=clean.filter(x=>(Number(x.ts)-startTs)<=40*60*1000);
    const heating=early.filter(x=>Number(x.l2_signal_w)>=1800&&Number(x.l2_signal_w)<=2700);
    const afterHeatStart=heating.length?Number(heating[0].ts):startTs;
    const after=clean.filter(x=>Number(x.ts)>=afterHeatStart);
    const work=after.filter(x=>Number(x.l2_signal_w)>=250&&Number(x.l2_signal_w)<=800);
    const rest=after.filter(x=>Number(x.l2_signal_w)>=40&&Number(x.l2_signal_w)<=260);
    const tail=after.filter(x=>(Number(x.ts)-startTs)>=60*60*1000&&Number(x.l2_signal_w)>=100&&Number(x.l2_signal_w)<=300);
    let alternations=0,lastBand=null;
    for(const x of after){const band=bandOf(Number(x.l2_signal_w));if((band==='WORK'||band==='REST')&&lastBand&&band!==lastBand)alternations++;if(band==='WORK'||band==='REST')lastBand=band;}
    const hasHeating=heating.length>=1,hasWork=work.length>=2,hasRest=rest.length>=1,hasAlternation=alternations>=2,durationEvidence=elapsedMin>=10,tailEvidence=tail.length>=1,directNow=directQuality.direct_active;
    const sequenceEvidence=[hasHeating,hasWork,hasRest,hasAlternation,durationEvidence].filter(Boolean).length;
    if(sequenceEvidence<4||!hasWork||!hasRest||!hasAlternation)return null;
    let confidence=anchorMode==='AEG'?0.42:0.50;
    if(hasHeating)confidence+=0.12;if(hasWork)confidence+=Math.min(0.10,work.length*0.025);if(hasRest)confidence+=0.05;if(hasAlternation)confidence+=Math.min(0.10,alternations*0.025);if(elapsedMin>=45)confidence+=0.07;if(tailEvidence)confidence+=0.05;if(directNow)confidence+=0.08;
    confidence=Math.min(confidence,anchorMode==='P1_SEQUENCE'&&!directNow?0.80:0.86);confidence=clamp01(confidence);
    ...
    const compensated=clean.filter(x=>x.boiler_compensated).length;
    return {
      id:'WASMACHINE_CANDIDATE',label:'Wasmachine',active:!cycleLooksComplete,
      confidence:Number(confidence.toFixed(2)),confidence_level:confidence>=0.68?'MEDIUM':'LOW',
      power_w:null,attribution_w:null,method:'SEQUENTIAL_L2_FINGERPRINT',validation_status:'SECOND_RUN_VALIDATION',phase:'L2',
      status_conflict:sourceConflict,fallback_allowed:fallbackAllowed,direct_source_quality:directQuality,anchor_mode:anchorMode,
      sequence:{elapsed_min:Number(elapsedMin.toFixed(1)),heating_samples:heating.length,work_samples:work.length,rest_samples:rest.length,work_rest_alternations:alternations,tail_samples:tail.length,boiler_compensated_samples:compensated,direct_aeg_active:directNow,cycle_complete_candidate:cycleLooksComplete},
      evidence:[
        'L2 bruto gereconstrueerd als P1_L2 + GoodWe4200',
        compensated?`${compensated} sample(s) gecorrigeerd voor direct gemeten boiler op L2`:'geen boilercorrectie nodig',
        hasHeating?`${heating.length} verwarmingspuls(en) 1,8–2,7 kW`:'geen vroege verwarmingspuls gezien',
        `${work.length} werkblok(ken) 0,25–0,80 kW`,`${rest.length} rustsample(s)`,`${alternations} werk/rust-overgang(en)`,
        sourceConflict?'SOURCE_CONFLICT: AEG meldt Idle terwijl sequentie sterk is':(fallbackAllowed?`AEG ${directQuality.status.toLowerCase()}: fallback toegestaan`:'AEG-status ondersteunt cyclus'),
        'geen vermogensattributie tijdens validatie'
      ],inferred:true
    };
  }

  function ovenCandidate(raw,history){
    if(isActive(raw?.loads?.quooker)||isActive(raw?.loads?.washer))return null;const current=residualPower(raw);if(current===null)return null;
    const recent=history.filter(x=>!x.quooker_active&&!x.washer_active&&nowFromRaw(raw)-Number(x.ts)<=45*60*1000&&finite(x.residual_w));if(recent.length<4)return null;
    const high=recent.filter(x=>Number(x.residual_w)>=1200),low=recent.filter(x=>Number(x.residual_w)<=700);let transitions=0;for(let i=1;i<recent.length;i++)if(Math.abs(Number(recent[i-1].residual_w)-Number(recent[i].residual_w))>=900)transitions++;
    const cyclic=high.length>=2&&low.length>=1&&transitions>=2;if(!cyclic||current<900)return null;const confidence=clamp01(0.58+Math.min(0.18,transitions*0.04)+Math.min(0.12,high.length*0.03));
  ...
  window.LoadFingerprintClassifier={version:VERSION,state:null,classify,refresh:()=>publish(),l2WasherSignal};
