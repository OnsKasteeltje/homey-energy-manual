...
function placeDayPicker(){
  ...
  // History redraws #eh-content whenever another day is selected. Any previous
  // KPI set was moved into this row and would otherwise remain here alongside
  // the newly rendered cards. Keep the summary row idempotent: exactly one
  // current card set followed by exactly one day selector.
  ...
  // Enforce stable ordering even after repeated redraw/mutation cycles.
...
document.addEventListener('change',e=>{
  if(e.target.matches('#energy-history-dashboard #eh-day-select')) schedule();
});
