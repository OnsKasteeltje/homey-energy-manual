(function(){
  ...
  // v1.1.1 can establish a conservative electrical cycle anchor when AEG falsely remains Idle.
  const VERSION='1.1.1';
  ...
  function blindAnchor(recent){
    // Used only when AEG stays Idle. It never anchors on a single high pulse.
    // We need a clean multi-sample WORK/REST pattern, optionally preceded by HEAT.
    const clean=cleanWasherRows(recent);
    if(clean.length<5)return null;
    for(let i=0;i<=clean.length-5;i++){
      const window=clean.slice(i);
      const bands=window.map(x=>bandOf(Number(x.l2_w))).filter(Boolean);
  ...
  function washerSequenceCandidate(raw,history){
    const ts=nowFromRaw(raw),recent=history.filter(x=>finite(x?.ts)&&ts-Number(x.ts)<=160*60*1000);
    if(recent.length<5)return null;

    let anchor=-1,anchorMode='AEG';
    for(let i=0;i<recent.length;i++)if(recent[i].washer_active){anchor=i;break;}
    if(anchor<0){
      const blind=blindAnchor(recent);if(!blind)return null;
      anchor=recent.findIndex(x=>Number(x.ts)>=blind.ts);anchorMode=blind.mode;
    }
    const cycle=recent.slice(anchor);if(cycle.length<4)return null;
    const clean=cleanWasherRows(cycle);if(clean.length<4)return null;
    ...
    for(const x of after){const band=bandOf(Number(x.l2_w));if((band==='WORK'||band==='REST')&&lastBand&&band!==lastBand)alternations++;if(band==='WORK'||band==='REST')lastBand=band;}

    const hasHeating=heating.length>=1,hasWork=work.length>=2,hasRest=rest.length>=1,hasAlternation=alternations>=2,durationEvidence=elapsedMin>=15,tailEvidence=tail.length>=1,directNow=isActive(raw?.loads?.washer);
    ...
    if(anchorMode==='P1_SEQUENCE'&&!directNow)confidence=Math.min(confidence,0.78);
    else confidence=Math.min(confidence,0.84);
    confidence=clamp01(confidence);

    const aegIdleConflict=anchorMode==='P1_SEQUENCE'&&!directNow;
    const cycleLooksComplete=anchorMode==='AEG'&&!directNow&&cycle.some(x=>x.washer_active)&&elapsedMin>=70;
    return {id:'WASMACHINE_CANDIDATE',label:'Wasmachine',active:!cycleLooksComplete,confidence:Number(confidence.toFixed(2)),confidence_level:confidence>=0.68?'MEDIUM':'LOW',power_w:null,attribution_w:null,method:'SEQUENTIAL_L2_FINGERPRINT',validation_status:'SECOND_RUN_VALIDATION',phase:'L2',status_conflict:aegIdleConflict,anchor_mode:anchorMode,sequence:{elapsed_min:Number(elapsedMin.toFixed(1)),heating_samples:heating.length,work_samples:work.length,rest_samples:rest.length,work_rest_alternations:alternations,tail_samples:tail.length,direct_aeg_active:directNow,cycle_complete_candidate:cycleLooksComplete},evidence:[hasHeating?`${heating.length} L2 verwarmingspuls(en) 1,8–2,6 kW`:'geen vroege verwarmingspuls gezien',`${work.length} L2 werkblok(ken) 0,25–0,70 kW`,`${rest.length} lage/rustsample(s)`,`${alternations} werk/rust-overgang(en)`,`${Math.round(elapsedMin)} min sinds ${anchorMode==='AEG'?'AEG':'P1-sequentie'}-anker`,aegIdleConflict?'AEG meldt Idle: bronconflict, inferred fallback':'AEG-status ondersteunt cyclus',tailEvidence?'L2 staartfase 0,10–0,26 kW gezien':'staartfase nog niet gezien','geen vermogensattributie tijdens validatie'],inferred:true};
  }
