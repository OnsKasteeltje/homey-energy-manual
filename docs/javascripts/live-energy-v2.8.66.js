(function(){
  ...
  function appliance(load){
    ...
    return {known,power,active,value:known?fmt(power):'—',sub:active?'actief':stateActive?(known?'stand-by / laag verbruik':'actief · vermogen niet apart gemeten'):'niet actief'};
  }
  ...
  function renderRaw(r,freshness){
    ...
    const quattDemand=thermal>100||working||(quatt>100&&thermo);
    const quattFlowActive=isActive(quatt);
    let heatSub='geen warmtevraag';
    if(quattFlowActive&&cvFlame)heatSub='Quatt + CV · hybride'; else if(quattFlowActive&&cvReq&&!cvKnown)heatSub='Quatt · CV gevraagd'; else if(quattFlowActive)heatSub='Quatt actief'; else if(cvFlame)heatSub='CV verwarmt'; else if(cvReq)heatSub='CV ondersteuning gevraagd'; else if(thermo||quattDemand)heatSub='warmtevraag · laag elektrisch verbruik';
    ...
    const consumers=[
      {x:55,title:'Tesla',value:fmt(tesla),sub:isActive(tesla)?'laden':(t.connected?'aangesloten · geen actief verbruik':'niet aangesloten'),w:tesla,active:isActive(tesla),ico:'car'},
      {x:295,title:'Boiler',value:fmt(boiler),sub:isActive(boiler)?'verwarmt':(hw.boiler_on?'aan · geen actief verbruik':'uit'),w:boiler,active:isActive(boiler),ico:'boiler'},
    ...
    const leftHeat=activeW(tesla)+activeW(boiler)+activeW(quatt);
    const leftBoiler=activeW(tesla)+activeW(boiler);
    const leftTesla=activeW(tesla);
    const rightWasher=activeW(washer.power)+activeW(dryer.power)+activeW(other);
    const rightDryer=activeW(dryer.power)+activeW(other);
    const rightOther=activeW(other);
    const total=leftHeat+rightWasher;
    const busY=505;
    svg+=busPath(`M750 410 V${busY}`,total,'energy-feed');
    svg+=busPath(`M750 ${busY} H630`,leftHeat,'energy-left-to-heating');
    svg+=busPath(`M630 ${busY} H390`,leftBoiler,'energy-heating-to-boiler');
    svg+=busPath(`M390 ${busY} H150`,leftTesla,'energy-boiler-to-tesla');
    svg+=busPath(`M750 ${busY} H870`,rightWasher,'energy-right-to-washer');
    svg+=busPath(`M870 ${busY} H1110`,rightDryer,'energy-washer-to-dryer');
    svg+=busPath(`M1110 ${busY} H1350`,rightOther,'energy-dryer-to-other');
    consumers.forEach(c=>{const cx=c.x+95;svg+=path(`M${cx} ${busY} V570`,activeW(c.w),'grid',c.active,'',true);svg+=smallNode(c.x,570,190,145,c.title,c.value,c.sub,c.title==='Overig'?'load residual':'load',c.ico,c.active);});
    ...
    const quattState=quattFlowActive?'verwarmt':thermo?'warmtevraag':'geen warmtevraag';
    ...
    const manager=`<div class="energy-manager-panel"><div class="energy-manager-title"><strong>Energiemanager</strong><span>functionele status</span></div><div class="energy-manager-grid"><div><small>TESLA</small><strong>${isActive(tesla)?'● Laden':'○ Niet actief'}</strong><span>${esc(t.need||r.manager?.decision||'HOLD')}</span></div><div><small>BOILER</small><strong>${hw.day_state?.goalReachedToday||hw.day_state?.goalReached?'● Dagdoel bereikt':isActive(boiler)?'● Verwarmt':'○ Geen actief verbruik'}</strong><span>${esc(hw.control?.reason||'')}</span></div><div><small>RUIMTEVERWARMING</small><strong>${quattFlowActive&&cvReq?'● Hybride':quattFlowActive?'● Quatt verwarmt':cvReq?'● CV gevraagd':'○ Geen actief elektrisch verbruik'}</strong><span>${esc(heatSub)}</span></div></div></div>`;
