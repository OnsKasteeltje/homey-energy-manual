(function(){
  ...
  function normalize(raw){
    if(!raw||raw?.meta?.schema_version!=='2.0') return null;
    ...
    return {
      ...
      stateFresh:stateAge!==null&&stateAge<=MAX_STATE_AGE_MS,
      heartbeatFresh:heartbeatAge!==null&&heartbeatAge<=MAX_HEARTBEAT_AGE_MS,
      healthy:heartbeatAge!==null&&heartbeatAge<=MAX_HEARTBEAT_AGE_MS,
  ...
  async function load(){
    try{
      ...
      const parsed=normalize(await r.json());
      if(!parsed)throw new Error('ongeldig Energy State v2-contract');
      current=parsed;
      window.EnergyCoreV2={available:true,state:current,refresh:load};
      document.dispatchEvent(new CustomEvent('energycorev2state',{detail:current}));
      return current;
    }catch(e){
      window.EnergyCoreV2={available:false,state:current,error:String(e?.message||e),refresh:load};
      return null;
    }
  }
  ...
  window.EnergyCoreV2={available:false,state:null,refresh:load};
