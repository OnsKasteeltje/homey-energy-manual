(function(){
  ...
  const applianceSub=a=>`${a.phase?`fase ${a.phase} · `:''}${a.w!==null?'live gemeten':a.active!==null?'status uit Homey':'geen live status gepubliceerd'}`;
  async function load(){
    ...
    try{
      ...
      const washer=appliance(bs,'washer','L2'),dryer=appliance(bs,'dryer','L3'),dishwasher=appliance(bs,'dishwasher','?');
      const measuredHeavy=[washer.w,dryer.w,dishwasher.w].filter(v=>v!==null).reduce((a,b)=>a+Math.max(0,b),0),otherHouse=Math.max(0,normal-measuredHeavy);
      ...
      svg+=flow(500,335,150,500,normal,normal>0,normal>0?fmtW(normal):'')+flow(515,335,400,500,tesla,tesla>0,tesla>0?fmtW(tesla):'')+flow(535,335,650,500,boiler,boiler>0,boiler>0?fmtW(boiler):'');if(exportW>0)svg+=flow(555,335,900,500,exportW,true,fmtW(exportW));svg+=control(465,132,400,500,'sturing')+control(525,132,650,500,'sturing');
      svg+=`<text x="430" y="630" text-anchor="middle" class="energy-layer-title">GROOTVERBRUIKERS BINNEN HUISHOUDEN</text>`;
      svg+=node(20,660,190,90,'Wasmachine',applianceValue(washer),applianceSub(washer),'house-subload')+node(230,660,190,90,'Droger',applianceValue(dryer),applianceSub(dryer),'house-subload')+node(440,660,190,90,'Vaatwasser',applianceValue(dishwasher),applianceSub(dishwasher),'house-subload')+node(650,660,190,90,'Overig huishouden',fmtW(otherHouse),measuredHeavy>0?'rest na gemeten grootverbruikers':'sluitpost huishouden','house-subload');
      svg+=flow(150,590,115,660,washer.w||0,washer.w!==null&&washer.w>0,washer.w?fmtW(washer.w):'')+flow(150,590,325,660,dryer.w||0,dryer.w!==null&&dryer.w>0,dryer.w?fmtW(dryer.w):'')+flow(150,590,535,660,dishwasher.w||0,dishwasher.w!==null&&dishwasher.w>0,dishwasher.w?fmtW(dishwasher.w):'')+flow(150,590,745,660,otherHouse,otherHouse>0,otherHouse>0?fmtW(otherHouse):'');
