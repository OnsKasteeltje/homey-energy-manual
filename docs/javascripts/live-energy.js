(function(){
  ...
  function flow(x1,y1,x2,y2,w,active=true,label=''){
    const width=active?clamp(2+Math.abs(w)/700,2.5,11):2, lx=(x1+x2)/2, ly=(y1+y2)/2-7;
    return `<g><line class="energy-path ${active?'is-active':'is-idle'}" x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" style="stroke-width:${width}" marker-end="url(#arrow)"/>${label?`<text x="${lx}" y="${ly}" text-anchor="middle" class="energy-edge-label">${label}</text>`:''}</g>`;
  }
  function control(x1,y1,x2,y2,label=''){
    const lx=(x1+x2)/2,ly=(y1+y2)/2-5;
    return `<g><line class="energy-control-path" x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" marker-end="url(#controlArrow)"/>${label?`<text x="${lx}" y="${ly}" text-anchor="middle" class="energy-control-label">${label}</text>`:''}</g>`;
  }
  ...
  async function load(){
    ...
    try{
      ...
      svg+=`<text x="525" y="24" text-anchor="middle" class="energy-layer-title">BESTURING / ORCHESTRATIE</text>`;
      svg+=node(405,40,240,92,'ENERGY MANAGER','SHADOW',String(bs.status||'Homey orchestratie'),'manager');
      ...
      svg+=node(55,245,190,90,'PV-bronnen',fmtW(pv),`SE · GW4200 · GW2000`,'source');
      svg+=node(430,245,190,90,'HUISBUS',fmtW(houseTotal),'centrale elektrische balans','bus');
      svg+=node(805,245,190,90,'GRID / P1',fmtW(Math.abs(p1)),p1>0?'import':p1<0?'export':'in balans','grid');
      svg+=node(55,520,190,90,'Huishouden',fmtW(normal),'prioriteit #1','load');
      svg+=node(305,520,190,90,'Tesla',fmtW(tesla),'prioriteit #2','load');
      svg+=node(555,520,190,90,'Boiler',fmtW(boiler),String(bs.boilerState||'—'),'load');
      svg+=node(805,520,190,90,'Netto net',p1>=0?fmtW(importW):fmtW(exportW),p1>=0?'afname':'teruglevering','sink');
      if(pv>0)svg+=flow(245,290,430,290,pv,true,fmtW(pv));
      if(importW>0)svg+=flow(805,300,620,300,importW,true,fmtW(importW));
      if(exportW>0)svg+=flow(620,280,805,280,exportW,true,fmtW(exportW));
      svg+=flow(500,335,150,520,normal,normal>0,normal>0?fmtW(normal):'');
      svg+=flow(515,335,400,520,tesla,tesla>0,tesla>0?fmtW(tesla):'');
      svg+=flow(535,335,650,520,boiler,boiler>0,boiler>0?fmtW(boiler):'');
      if(exportW>0)svg+=flow(555,335,900,520,exportW,true,fmtW(exportW));
      svg+=control(465,132,400,520,'sturing');svg+=control(525,132,650,520,'sturing');
      svg+=`<text x="525" y="670" text-anchor="middle" class="energy-rule">Toewijzingslogica: huishouden → Tesla → boiler → net</text>`;
      svg+=`<text x="525" y="698" text-anchor="middle" class="energy-rule">Energy Manager ligt buiten het elektrische stroompad en stuurt alleen flexibele verbruikers.</text>`;
      svg+='</svg>';
      root.innerHTML=`<div class="energy-topline"><span><strong>Laatste data:</strong> ${newest?new Date(newest).toLocaleString('nl-NL'):'onbekend'}</span><span class="${stale?'energy-stale':'energy-ok'}">${stale?'● data ouder dan 30 min':'● actueel'}</span></div>${svg}<div class="energy-summary"><div><strong>PV</strong><br>${fmtW(pv)}</div><div><strong>Huishouden totaal</strong><br>${fmtW(houseTotal)}</div><div><strong>Grid</strong><br>${p1>=0?`${fmtW(importW)} import`:`${fmtW(exportW)} export`}</div><div><strong>M7</strong><br>${String(ml.advice||'—')}</div></div><p class="energy-footnote">Doorgetrokken dikke lijnen tonen energie. Dunne gestuurde lijnen vanuit de Energy Manager tonen orchestratie en zijn géén elektrische verbindingen. De verdeling over verbruikers is boekhoudkundig afgeleid uit P1, PV en gemeten apparaatvermogens.</p>`;
