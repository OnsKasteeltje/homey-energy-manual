(function(){
  ...
  function renderRaw(r, freshness){
    ...
    for(const k of knownLoadKeys){const x=loads[k]; if(!x)continue; if(x.power_w!==null&&x.power_w!==undefined&&Number.isFinite(Number(x.power_w)))knownAppliances+=pos(x.power_w); else if(x.active===true)unmetered.push(k);}
    ...
    const otherSub=unmetered.length?`rest; actief maar niet watt-gemeten: ${unmetered.join(', ')}`:'rest na alle bekende vermogens';
    ...
    root.innerHTML=`
      ...
      <div class="energy-manager-panel"><div class="energy-manager-title"><strong>Elektrisch verbruik woning</strong><span>vier parallelle verbruikstakken</span></div><div class="energy-manager-grid energy-load-grid">
      ...
      <p class="energy-rule">Overig = woning − Tesla − boiler − Quatt − alle overige individueel gemeten <code>loads.*.power_w</code>. Actieve apparaten zonder wattmeting blijven zichtbaar als onzeker deel van Overig.</p>`;
  }
