(function(){
  ...
  const RESIDUAL_LATCH_KEY='em2-washer-residual-fallback-v1';
  const RESIDUAL_MIN_W=900;
  const RESIDUAL_MAX_W=2900;
  const RESIDUAL_CONFIRM_MS=4*60*1000;
  const RESIDUAL_MAX_GAP_MS=8*60*1000;
  ...
  function readResidualLatch(){
    try{return JSON.parse(localStorage.getItem(RESIDUAL_LATCH_KEY)||'null');}catch(_){return null;}
  }
  function writeResidualLatch(v){
    try{if(v)localStorage.setItem(RESIDUAL_LATCH_KEY,JSON.stringify(v));else localStorage.removeItem(RESIDUAL_LATCH_KEY);}catch(_){}
  }
  function currentResidual(raw){
    if(raw?.balance?.derived_house_balance_valid===false) return null;
    if(finite(raw?.energy_budget?.other_house_load_w)) return Math.max(0,Number(raw.energy_budget.other_house_load_w));
    if(finite(raw?.balance?.residual_w)) return Math.max(0,Number(raw.balance.residual_w));
    return null;
  }
  function residualContextSafe(raw,washer){
    const s=washer?.state||{};
    const connected=String(s.connectionState||'').toLowerCase()==='connected';
    const idle=/^(idle|off|inactive)$/i.test(String(s.applianceState||''));
    const dryer=raw?.loads?.dryer;
    const dryerActive=dryer?.active===true||dryer?.state?.active===true;
    const quookerActive=raw?.loads?.quooker?.active===true;
    const teslaActive=raw?.tesla?.charging===true||(finite(raw?.tesla?.power_w)&&Number(raw.tesla.power_w)>20);
    const heatingActive=finite(raw?.quatt?.power_w)&&Number(raw.quatt.power_w)>250;
    return connected&&idle&&!dryerActive&&!quookerActive&&!teslaActive&&!heatingActive;
  }
  function applyResidualFallback(){
    const state=window.EnergyCoreV2?.state;
    const raw=state?.raw;
    const washer=raw?.loads?.washer;
    if(!raw||!washer||washer.active===true) return;
    const residual=currentResidual(raw);
    const now=Date.parse(raw?.meta?.generated_at||'')||Date.now();
    const qualifies=residualContextSafe(raw,washer)&&residual!==null&&residual>=RESIDUAL_MIN_W&&residual<=RESIDUAL_MAX_W;
    let latch=readResidualLatch();
    if(!qualifies){writeResidualLatch(null);return;}
    if(!latch||!finite(latch.first_at)||now-Number(latch.last_at)>RESIDUAL_MAX_GAP_MS){
      latch={first_at:now,last_at:now,samples:1,min_w:residual,max_w:residual};
      writeResidualLatch(latch);
      return;
    }
    latch.last_at=now;
    latch.samples=Number(latch.samples||0)+1;
    latch.min_w=Math.min(Number(latch.min_w)||residual,residual);
    latch.max_w=Math.max(Number(latch.max_w)||residual,residual);
    writeResidualLatch(latch);
    if(now-Number(latch.first_at)<RESIDUAL_CONFIRM_MS||latch.samples<2) return;

    const candidate={
      confidence:0.68,
      phase:'L2',
      method:'PERSISTENT_RESIDUAL_FALLBACK',
      direct_source_quality:{status:'HEALTHY_IDLE'}
    };
    applyWasherInferredActive(state,washer,candidate,raw,{source_revision:raw?.meta?.state_revision??null},'AEG_IDLE_DEGRADED_BY_PERSISTENT_UNATTRIBUTED_LOAD');
    raw.meta.laundry_fingerprint_fallback.residual_w=Math.round(residual);
    raw.meta.laundry_fingerprint_fallback.samples=latch.samples;
    raw.meta.laundry_fingerprint_fallback.duration_sec=Math.round((now-Number(latch.first_at))/1000);
  }
  ...
  document.addEventListener('energycorev2state',()=>setTimeout(applyResidualFallback,30));
