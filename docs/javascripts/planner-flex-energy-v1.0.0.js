(() => {
  ...
  const fixBoilerRow=ww=>{
    ...
    if(!section)return;
    let row=[...section.querySelectorAll('.ps-action-row')].find(r=>r.querySelector('.ps-action-name')?.textContent?.trim()==='Boiler');
    if(!row){row=document.createElement('div');row.className='ps-action-row';const name=document.createElement('div');name.className='ps-action-name';name.textContent='Boiler';const track=document.createElement('div');track.className='ps-action-track';row.append(name,track);section.append(row);}
