(function(){
  ...
  function renderRaw(r,freshness){
    ...
    consumers.forEach(c=>{const cx=c.x+115;svg+=path(`M${c.sx} 395 V475 H${cx} V535`,c.w,'grid',c.active);svg+=node(c.x,535,230,115,'VERBRUIK',c.title,c.value,c.sub,'load');});

    // Overig krijgt bewust een eigen route ONDER de verbruikersrij door, zodat de lijn niet door Ruimteverwarming loopt.
    svg+=path('M890 395 V705 H1355 V650',other,'grid',other>0);
    svg+=node(1240,535,230,115,'VERBRUIK','Overig',fmt(other),otherSub,'load residual');
    svg+=`<text x="750" y="785" text-anchor="middle" class="energy-rule">Zes onafhankelijke verbruikstakken · Overig loopt visueel onder de verbruikersrij door en blijft de restcategorie.</text></svg>`;

    let cvState='CV-status onbekend';
    if(cvFlame)cvState='vlam actief'; else if(cvReq)cvState=cvKnown?'ondersteuning aangestuurd':'ondersteuning gevraagd · vlam onbekend'; else cvState=cvKnown?'niet actief':'niet gevraagd · vlam onbekend';
    const quattState=quattActive?'verwarmt':thermo?'warmtevraag':'geen warmtevraag';
    const heatExtra=[thermal>0?`thermisch ${fmt(thermal)}`:'',cop?`COP ${cop.toFixed(1)}`:''].filter(Boolean).join(' · ');
