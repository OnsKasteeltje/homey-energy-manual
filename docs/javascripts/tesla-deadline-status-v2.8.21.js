(function(){
  const LABELS={
    ...
    DEADLINE_PV_FORECAST:'Laden · gunstig PV-forecastuur',
  ...
  function apply(){
    const panel=document.getElementById('tesla-deadline-control');if(!panel)return;
    const box=[...panel.querySelectorAll('.tesla-deadline-runtime > div')].find(d=>d.querySelector('span')?.textContent.trim()==='Status');
    const strong=box?.querySelector('strong');if(!strong)return;
    const raw=strong.textContent.trim();if(LABELS[raw])strong.textContent=LABELS[raw];
  }
  function start(){apply();const root=document.getElementById('live-energy-flow')||document.body;new MutationObserver(apply).observe(root,{childList:true,subtree:true,characterData:true});}
