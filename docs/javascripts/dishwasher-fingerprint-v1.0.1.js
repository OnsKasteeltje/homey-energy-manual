(function(){
  ...
  // Validated installation phase map: SolarEdge=L1, GoodWe4200=L2, GoodWe2000=L3.
  // 2026-08-22 ground truth added: active cycle included ~2 kW HEAT and ~80 W tail,
  // followed by confirmed OFF with gross L1 around 134 W. Low-load tail therefore
  // needs sequence context; instantaneous 80-180 W alone must never keep the cycle active.
  const VERSION='1.0.1';
  const HISTORY_KEY='em2-dishwasher-fingerprint-v2';
  ...
  // Non-overlapping bands. OFF/BASELINE deliberately captures the validated
  // post-cycle region that previously overlapped PUMP.
  ...
  function classify(raw){
    ...
    if(currentBand==='HEAT'){
      confidence=0.76;evidence.push(`L1 verwarmingsband ${Math.round(sig.signal_w)} W`,'geen bekende grote conflicterende belasting actief');
      if(heat>=2){confidence+=0.05;evidence.push(`${heat} verwarmingssamples`);}if(pump>=1){confidence+=0.07;evidence.push(`${pump} pomp/werk-sample(s)`);}if(transitions>=2){confidence+=0.05;evidence.push(`${transitions} fase-overgang(en)`);}
    }else if(currentBand==='PUMP'&&heat>=1&&sinceStrongMs<=45*60*1000){
      ...
      confidence=0.70+Math.min(0.08,transitions*0.02);evidence.push('lage staartfase kort na bevestigde werkfase','2026-08-22 gevalideerde staart kan ~80 W zijn','lage belasting nog niet lang genoeg voor OFF-bevestiging');
    }else if((currentBand==='TAIL_LOW'||currentBand==='OFF_LOW')&&heat>=1&&pump>=1&&(sinceStrongMs>TAIL_MAX_AGE_MS||lowSpanMs>=OFF_CONFIRM_MS)){
      return {version:VERSION,active:false,confidence:0.93,power_w:0,reason:'CYCLE_END_CONFIRMED',signal:sig,sequence:{heat,pump,tail,transitions,since_strong_min:Number((sinceStrongMs/60000).toFixed(1)),low_span_min:Number((lowSpanMs/60000).toFixed(1))},evidence:['ground-truth OFF-regel 2026-08-22','aanhoudend lage L1-belasting na eerdere HEAT+PUMP-sequentie']};
    }else{
