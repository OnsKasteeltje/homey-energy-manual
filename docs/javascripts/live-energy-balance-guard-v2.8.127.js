(function(){
  ...
  const fmtGrid=v=>{
    const n=num(v);
    if(n===null)return 'P1 —';
    if(n<0)return `${fmt(Math.abs(n))} export`;
    if(n>0)return `${fmt(n)} import`;
    return '0 W net';
  };
  ...
  function setPanel(root,mode,house,grid,measuredAssigned,estimatedAssigned,other){
    ...
    if(mode==='valid'){
      ...
      if(span)span.textContent=house!==null?`indicatieve weergave: PV + netimport − netexport + batterijontlading − batterijlading · P1/PV niet tijdgelijk · geen control-impact`:'P1/PV niet tijdgelijk · huisbalans niet beschikbaar · geen control-impact';
    }else{
  ...
  function apply(){
    ...
    try{
      ...
      const physicalValid=gate===false?false:(rawHousePlausible&&rawHouse+TOLERANCE_W>=assigned);
      ...
      if(physicalValid){
        setNode(houseNode,fmt(house),'verbruik = PV + netimport − netexport + batterijontlading − batterijlading');
        ...
        if(house!==null)setNode(houseNode,`≈ ${fmt(house)}`,'indicatief verbruik uit PV + P1; bronnen niet tijdgelijk');
        else setNode(houseNode,'—','huisverbruik tijdelijk niet betrouwbaar te bepalen');
        setNode(otherNode,'—','restverbruik niet betrouwbaar te bepalen bij bron-skew');
      }else{
        setNode(houseNode,'—','weergavebalans onzeker');
        setNode(otherNode,'—','detailtoewijzing onzeker · geen control-impact');
      }
      setPanel(root,mode,house,grid,measuredAssigned,estimatedLoads,other);setWarning(root,mode);
      root.dataset.balanceStatus=mode;
      root.dataset.balanceMeasurementPolicy=mode==='async'?'p1-pv-indicative-house-control-gated':'core-derived-gate-authoritative';
      root.dataset.balanceControlImpact='none';
      root.dataset.balanceGuardVersion='2.8.127';
    }finally{applying=false;}
  }

  // House is always a consumption quantity. Grid remains the only import/export quantity.
  // During source skew we may show an explicitly approximate presentation balance, but Core's
  // derived gate remains authoritative and this display-only estimate has zero control impact.
