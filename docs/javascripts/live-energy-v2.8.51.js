(function(){
  ...
  function path(d,w,kind='grid',active=true,label='',lx=0,ly=0){
    const width=active?clamp((2.5+Math.abs(n(w))/750)*1.2,3.6,10.8):2.4;
  ...
  function renderRaw(r,freshness){
    ...
    const copVals=[Number(q.cop_1),Number(q.cop_2)].filter(v=>Number.isFinite(v)&&v>0); const cop=copVals.length?copVals.reduce((a,v)=>a+v,0)/copVals.length:null;
    ...
    svg+=path('M240 135 V180 H750 V245',se,'pv',se>0); svg+=path('M750 135 V245',gw42,'pv',gw42>0); svg+=path('M1260 135 V180 H750 V245',gw20,'pv',gw20>0);
    ...
    const consumers=[{x:15,title:'Tesla',value:fmt(tesla),sub:t.charging?'laden':(t.connected?'aangesloten':'niet aangesloten'),w:tesla,active:tesla>0,sx:610},{x:260,title:'Boiler',value:fmt(boiler),sub:hw.boiler_on?'aan':'uit',w:boiler,active:boiler>0,sx:665},{x:505,title:'Ruimteverwarming',value:fmt(quatt),sub:heatSub,w:quatt,active:quatt>0,sx:720},{x:750,title:'Wasmachine',value:washer.value,sub:washer.sub,w:washer.power,active:washer.known&&washer.power>0,sx:780},{x:995,title:'Droger',value:dryer.value,sub:dryer.sub,w:dryer.power,active:dryer.known&&dryer.power>0,sx:835},{x:1240,title:'Overig',value:fmt(other),sub:otherSub,w:other,active:other>0,sx:890}];
    ...
    svg+=`<text x="750" y="710" text-anchor="middle" class="energy-rule">Zes onafhankelijke verbruikstakken · iedere verbinding eindigt aan de bovenzijde van het eigen verbruikersblok.</text></svg>`;
    let cvState='CV-status onbekend'; if(cvFlame)cvState='vlam actief'; else if(cvReq)cvState=cvKnown?'ondersteuning aangestuurd':'ondersteuning gevraagd · vlam onbekend'; else cvState=cvKnown?'niet actief':'niet gevraagd · vlam onbekend';
    const quattState=quattActive?'verwarmt':thermo?'warmtevraag':'geen warmtevraag'; const heatExtra=[thermal>0?`thermisch ${fmt(thermal)}`:'',cop?`COP ${cop.toFixed(1)}`:''].filter(Boolean).join(' · '); const assigned=tesla+boiler+quatt+washer.power+dryer.power+otherKnownExtras;
    const hybrid=`<div class="energy-manager-panel heating-hybrid-panel"><div class="energy-manager-title"><strong>Verwarmingsopwekking · Quatt Hybrid</strong><span>één functioneel verwarmingssysteem</span></div><div class="energy-manager-grid"><div><small>QUATT WARMTEPOMP</small><strong>${fmt(quatt)} elektrisch</strong><span>${esc(quattState)}${heatExtra?` · ${esc(heatExtra)}`:''}</span></div><div><small>CV-KETEL</small><strong>${esc(cvState)}</strong><span>${cvReq?'CiC vraagt CV-ondersteuning':'geen CV-ondersteuning gevraagd'}</span></div><div><small>ENERGIEBALANS</small><strong>Woning ${fmt(house)}</strong><span>bekend toegewezen ${fmt(assigned)} · Overig ${fmt(other)}</span></div></div></div>`;
  ...
  document.addEventListener('energycorev2state',e=>renderDetail(e.detail)); document.addEventListener('DOMContentLoaded',()=>setTimeout(boot,200)); document.addEventListener('DOMContentSwitch',()=>setTimeout(boot,100)); setTimeout(boot,800);
})();