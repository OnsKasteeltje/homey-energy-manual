(() => {
  ...
  function showToast(text) {
    let toast = document.getElementById('app-refresh-toast');
    if (!toast) {
      toast = document.createElement('div');
      toast.id = 'app-refresh-toast';
      toast.setAttribute('role', 'status');
      Object.assign(toast.style, {
        position: 'fixed', left: '50%', top: 'calc(env(safe-area-inset-top, 0px) + 14px)',
        transform: 'translateX(-50%)', zIndex: '99999', padding: '9px 14px',
        borderRadius: '999px', background: 'rgba(20,24,33,.92)', color: '#fff',
        fontSize: '14px', fontWeight: '700', boxShadow: '0 4px 18px rgba(0,0,0,.25)',
  ...
  async function refreshData(reason='manual', force=false) {
    if (document.hidden && reason !== 'manual' && reason !== 'pull-to-refresh') return false;
    const now = Date.now();
  ...
  async function hardRefreshApp() {
    showToast('Vernieuwen…');
    await refreshData('pull-to-refresh', true);
    try {
      if ('serviceWorker' in navigator) {
        const registration = await navigator.serviceWorker.getRegistration();
        if (registration) await registration.update();
      }
      await fetch(location.href, { cache: 'no-store', headers: { 'Cache-Control': 'no-cache' } });
    } catch (_) {
      // Data-refresh is already done; reload still gives the browser a chance to update assets.
    }
    try { sessionStorage.setItem('home-energy-refresh-confirm', String(Date.now())); } catch (_) {}
    setTimeout(() => location.reload(), 250);
  }

  window.HomeEnergyApp = { version: '2.0.18', refreshData, hardRefreshApp, renderFreshness };
  ...
  function boot() {
    ...
    try {
      const refreshedAt = Number(sessionStorage.getItem('home-energy-refresh-confirm'));
      if (Number.isFinite(refreshedAt) && Date.now() - refreshedAt < 15000) {
        sessionStorage.removeItem('home-energy-refresh-confirm');
        setTimeout(() => showToast('Vernieuwd'), 250);
      }
    } catch (_) {}
