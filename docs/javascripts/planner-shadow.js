(() => {
  ...
  async function fetchJson(primary, rawFile, required=true){
    ...
    for(const url of candidates){
      try{const r=await fetch(url,{cache:'no-store'});if(!r.ok){last=`HTTP ${r.status} op ${url}`;continue;}return await r.json();}
  ...
  function renderScore(payload){
    ...
    const grid=el('div','ps-grid'); grid.append(
      ...
      card('Rekenscenario','Zonder saldering',`afname € ${fmt(BUSINESS_CASE.import2027,3)}/kWh · netto teruglevering € ${fmt(BUSINESS_CASE.netExport2027,4)}/kWh`),
    ...
    const q=el('div','ps-quality'); q.append(badge('SHADOW','good'),badge('CONTROL IMPACT: NONE','good'),badge('SCENARIO: ZONDER SALDERING','good'),badge(`PV forecast: ${i.pvForecast?.quality||'—'}`,i.pvForecast?.quality==='SUMMARY_ONLY'?'warn':''));
    if((p.plan?.slotsAvailable||0)===0) q.append(badge('Geen kwartierslots bij huidig FIXED-plan','warn')); wrap.append(q); return wrap;
  }

  function renderTimeline(payload){
    const p=payload.plan||{}, actions=p.plan?.actions||[]; const sec=el('section','ps-section'); sec.append(el('h2','', '24-uurs plan'));
    ...
    const scroller=el('div','ps-timeline-scroll'),table=el('table','ps-timeline'),trh=el('tr');['Tijd','Prijs','Batterij','Tesla','Warm water'].forEach(x=>trh.append(el('th','',x)));const thead=el('thead');thead.append(trh);table.append(thead);const tb=el('tbody');actions.forEach(a=>{const tr=el('tr');const cells=[new Date(a.start).toLocaleTimeString('nl-NL',{hour:'2-digit',minute:'2-digit'}),Number.isFinite(Number(a.price))?`€ ${Number(a.price).toFixed(3)}`:'—',a.battery||'HOLD',a.tesla||'HOLD',a.warmWater||'HOLD'];cells.forEach((x,j)=>{const td=el('td',j>1&&x!=='HOLD'?'ps-active':'',x);tr.append(td)});tb.append(tr)});table.append(tb);scroller.append(table);sec.append(scroller);return sec;
  }
  ...
  function simulateDay(day,cfg){
    ...
    for(let idx=0;idx<samples.length;idx++){const s=samples[idx],next=samples[idx+1];let dtH=5/60;if(next){const d=(new Date(next.ts)-new Date(s.ts))/3600000;if(d>0&&d<0.5)dtH=d;}const p=Number(s.p1W);
    ...
    const throughput=chargedGrid+dischargedGrid,avoidedImportValue=dischargedGrid*BUSINESS_CASE.import2027,lostExportCredit=chargedGrid*BUSINESS_CASE.netExport2027;
  ...
  function renderDecision(history,payload){
    const sec=el('section','ps-section ps-decision');sec.append(el('h2','', 'Potentiële besparing · zonder saldering'));
    const etaLeg=Math.sqrt(BUSINESS_CASE.roundTripEfficiency),mainCfg={usableKWh:BUSINESS_CASE.usableWindowKWh,maxW:BUSINESS_CASE.acLimitW,etaC:etaLeg,etaD:etaLeg},smallCfg={usableKWh:6.72,maxW:2500,etaC:etaLeg,etaD:etaLeg};
    ...
    const deltaEuro=main.netEuro-small.netEuro,deltaImport=main.dischargedGrid-small.dischargedGrid,simplePayback=BUSINESS_CASE.investmentEuro/BUSINESS_CASE.annualCentralEuro;
    sec.append(el('p','ps-footnote',`Alle bedragen op deze pagina gebruiken hetzelfde scenario zonder saldering: € ${fmt(BUSINESS_CASE.import2027,3)}/kWh netafname en € ${fmt(BUSINESS_CASE.netExport2027,4)}/kWh netto terugleverwaarde.`));
    ...
    sec.append(el('h3','',`Validatie op gemeten P1-data · zonder saldering · ${mainRows.length} dagen`));
    sec.append(el('p','ps-footnote','De gemeten dagen worden met exact hetzelfde scenario gewaardeerd. Dit is empirische validatie van de batterijwerking, geen extrapolatie naar een jaar.'));
    ...
    sec.append(el('p','ps-footnote','Direct PV-verbruik door Tesla, boiler en andere verbruikers wordt niet dubbel als batterijbesparing geteld: de replay gebruikt netto P1-export en -import. Batterijkeuze: 3 × Pylontech US5000 = 14,4 kWh; circa 10 kWh blijft energetisch de sweet spot, terwijl de derde module vooral de gewenste 3,0–3,3 kW AC-ESS-regeling en extra buffer mogelijk maakt.'));
    return sec;
  }

  function renderReplay(history,payload){
    const sec=el('section','ps-section');sec.append(el('h2','', 'Historische batterijreplay · zonder saldering'));
    const etaLeg=Math.sqrt(BUSINESS_CASE.roundTripEfficiency),cfg={usableKWh:BUSINESS_CASE.usableWindowKWh,maxW:BUSINESS_CASE.acLimitW,etaC:etaLeg,etaD:etaLeg},rows=replayRows(history,cfg).reverse();if(!rows.length){sec.append(el('div','ps-empty','Nog onvoldoende daghistorie voor replay.'));return sec;}const sum=aggregate(rows);
    const grid=el('div','ps-grid ps-replay-summary');grid.append(
      card('Scenario-effect',euro(sum.netEuro),`${euro(sum.avoidedImportValue)} vermeden inkoop − ${euro(sum.lostExportCredit)} gemiste teruglevering`),
    ...
    const table=el('table','ps-replay-table'),th=el('tr');['Dag','Export gemeten','Naar accu','Import gemeten','Vermeden import','€ effect','Verlies','Cycli'].forEach(x=>th.append(el('th','',x)));const head=el('thead');head.append(th);table.append(head);const tb=el('tbody');rows.forEach(r=>{const tr=el('tr');[r.date,fmtKWh(r.exportRaw),fmtKWh(r.chargedGrid),fmtKWh(r.importRaw),fmtKWh(r.dischargedGrid),euro(r.netEuro),fmtKWh(r.losses),fmt(r.cycles,2)].forEach(x=>tr.append(el('td','',x)));tb.append(tr)});table.append(tb);const sc=el('div','ps-timeline-scroll');sc.append(table);sec.append(sc);
    sec.append(el('p','ps-footnote','Replay gebruikt gemeten netto P1-data en waardeert alle dagen volgens hetzelfde scenario zonder saldering. Geen seizoensopslag of volledige jaarprognose.'));
    return sec;
  }

  async function load(){const status=document.getElementById('ps-status');try{const [payload,history]=await Promise.all([fetchJson(root.dataset.source,'energy-planner-shadow.json',true),fetchJson(root.dataset.history,'energy-day-series-7d.json',false)]);if(payload.observabilityOnly!==true||payload.controlImpact!=='NONE')throw new Error('observability safety marker ontbreekt');status.replaceChildren(renderDecision(history,payload),renderScore(payload),renderTimeline(payload),renderReplay(history,payload));}catch(e){status.replaceChildren(el('div','ps-error',`Planner Shadow kon niet worden geladen: ${e.message}`));}}
  load();
})();