(function(){
  ...
  let lastSnapshot=null,controlConfig=null;
  ...
  async function refreshData(){lastSnapshot=await getJson('data/pv-phase-24h.json').then(j=>(j?.samples||[]).at(-1)||null);controlConfig=await getJson('data/tesla-control-config.json');}
