(function(){
  ...
  // PRESENTATION BOUNDARY: this module is display-only.
  // It consumes a detached copy of the published Energy Core snapshot and must never
  // mutate EnergyCoreV2 state, Decision/Shadow data, control gates or actuator inputs.
  ...
  function setPanel(root,mode,house,measuredAssigned,estimatedAssigned,other){const panels=[...root.querySelectorAll('.heating-hybrid-panel .energy-manager-grid > div')],b=panels.find(x=>x.querySelector('small')?.textContent?.trim()==='ENERGIEBALANS');if(!b)return;const strong=b.querySelector('strong'),span=b.querySelector('span');if(mode==='valid'){if(strong)strong.textContent=`Woning ${fmt(house)}`;if(span){const estimatedPart=estimatedAssigned>0?` · indicatief toegewezen ${fmt(estimatedAssigned)}`:'';span.textContent=`top-level toegewezen ${fmt(measuredAssigned)}${estimatedPart} · Overig ${fmt(other)}`;}}else{if(strong)strong.textContent='Woning —';if(span)span.textContent='presentatiebalans onzeker · control blijft op Core-context';}}
  ...
  function apply(){
    ...
    try{
      ...
      // Presentation-only allocation. Never feed this derived result back into Core/Decision/Control.
      ...
      const physicalValid=Number.isFinite(rawHouse)&&rawHouse>=-TOLERANCE_W&&rawHouse+TOLERANCE_W>=assigned,mode=physicalValid?'valid':'invalid',house=physicalValid?Math.max(0,rawHouse):null,other=physicalValid?Math.max(0,house-assigned):null,houseNode=findNode(root,'Huis'),otherNode=findNode(root,'Overig');
      if(physicalValid){setNode(houseNode,fmt(house),'weergave uit gepubliceerde P1/PV-snapshot');setNode(otherNode,fmt(other),estimatedLoads>0?'weergaverest na directe + indicatieve toewijzing':'weergaverest na bekende top-level vermogens');}else{setNode(houseNode,'—','weergavebalans onzeker');setNode(otherNode,'—','geen control-impact');}
      setPanel(root,mode,house,measuredAssigned,estimatedLoads,other);setWarning(root,mode);
      root.dataset.balanceStatus=mode;
      root.dataset.balanceMeasurementPolicy='presentation-only';
      root.dataset.balanceControlImpact='none';
      root.dataset.balanceGuardVersion='2.8.120';
