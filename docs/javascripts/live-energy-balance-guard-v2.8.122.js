(function(){
  ...
  function setPanel(root,mode,displayHouse,measuredAssigned,estimatedAssigned,other){
    ...
    if(mode==='valid'){
      ...
      if(strong)strong.textContent=displayHouse!==null?`Woning ~${fmt(displayHouse)}`:'Woning —';
  ...
  function apply(){
    ...
    try{
      ...
      root.dataset.balanceGuardVersion='2.8.122';
    }finally{applying=false;}
  }

  // Alleen data onthouden bij Core-update. DOM-correctie gebeurt synchroon direct na de hoofd-render.
  // Hiermee verdwijnen de eerdere MutationObservers en 0/80/250/600/1200 ms repaint-rondes.
