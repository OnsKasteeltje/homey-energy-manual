  ...
  const boot=async root=>{
    ...
    try{
      if(!hasRendered(root)){
        ...
        if(!ok){await loadScript('../javascripts/planner-shadow.js?lifecycle=5');ok=await waitRendered(root,8000);}
        if(!ok){loaded.delete(root);return;}
      }
      await loadScript('../javascripts/planner-axes-v1.1.0.js?lifecycle=5').catch(()=>{});
      await loadScript('../javascripts/planner-ww-multislot-v1.0.0.js?lifecycle=5').catch(()=>{});
      await loadScript('../javascripts/planner-flex-energy-v1.0.0.js?lifecycle=5').catch(()=>{});
      await loadScript('../javascripts/planner-action-details-v1.0.1.js?lifecycle=5').catch(()=>{});
      await loadScript('../javascripts/planner-device-tabs-v1.0.3.js?lifecycle=5').catch(()=>{});
