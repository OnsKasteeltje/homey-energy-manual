// EV Actuator v0.4.2 PHASE-WRITER LIVE
...
// Easee Cloud is used only for phase-mode command and phase observation 38.
// Active electrical phase telemetry is authoritative when charging, so stale
// Homey settings readback cannot create a same-phase pause transition.

const VERSION='EM2_EV_ACTUATOR_V0.4.2_PHASE_WRITER';
...
const PHASE_CLOUD_CONFIRM_INTERVAL_MS=5000;
...
const findObservationValue=(node,id)=>{
  ...
  if(Array.isArray(node)){
    for(const child of node){const v=findObservationValue(child,id);if(v!==null)return v;}
    return null;
  }
  ...
  for(const child of Object.values(node)){const v=findObservationValue(child,id);if(v!==null)return v;}
  return null;
};
...
const homeyConfirmedMode=normalizePhaseMode(phaseRaw);
...
const electricalMode=
  charging===true && powerW!==null && powerW>500 && offeredA!==null && offeredA>=5.5
    ...
    :'UNKNOWN';
let confirmedMode=electricalMode!=='UNKNOWN'?electricalMode:homeyConfirmedMode;
let phaseConfirmationSource=electricalMode!=='UNKNOWN'?'ELECTRICAL_TELEMETRY':homeyConfirmedMode!=='UNKNOWN'?'HOMEY_SETTINGS':'NONE';
let cloudPhaseRaw=null;
const paused=
  charging!==true &&
...
let t=priorT||{
  ...
  stageSince:iso(),
  failure:null,
  phaseConfirmedMode:null,
  phaseConfirmedAt:null,
  phaseConfirmationSource:null,
  phaseCloudCheckedAt:null,
  phaseCloudError:null
};

if(t.stage!=='STABLE'&&t.phaseConfirmedMode===desiredMode){
  confirmedMode=desiredMode;
  phaseConfirmationSource=t.phaseConfirmationSource||'TRANSITION_LATCH';
}

const cloudConfirmStage=['CONFIRMING_PHASE','DEADTIME','ARMING_CIRCUIT_CAP'].includes(t.stage);
if(
  paused &&
  cloudConfirmStage &&
  confirmedMode!==desiredMode &&
  age(t.phaseCloudCheckedAt)>=PHASE_CLOUD_CONFIRM_INTERVAL_MS
){
  try{
    cloudPhaseRaw=await readCloudPhaseMode(vars);
    const cloudMode=normalizePhaseMode(cloudPhaseRaw);
    t={...t,phaseCloudCheckedAt:iso(),phaseCloudError:null};
    if(cloudMode===desiredMode){
      confirmedMode=cloudMode;
      phaseConfirmationSource='EASEE_CLOUD_OBSERVATION_38';
    }
  }catch(err){
    t={...t,phaseCloudCheckedAt:iso(),phaseCloudError:String(err?.message||err||'EASEE_CLOUD_PHASE_READ_FAILED')};
  }
}

if(t.stage!=='STABLE'&&confirmedMode===desiredMode&&t.phaseConfirmedMode!==desiredMode){
  t={
    ...t,
    phaseConfirmedMode:desiredMode,
    phaseConfirmedAt:iso(),
    phaseConfirmationSource
  };
}
...
if(t.stage==='STABLE'){
  ...
  if(confirmedMode===desiredMode&&paused){
    t={...t,stage:'ARMING_CIRCUIT_CAP',transitionId,requestedMode:desiredMode,requestedA:desiredA,originalCircuitA:circuitTargetA,startedAt:iso(),stageSince:iso(),failure:null,phaseConfirmedMode:desiredMode,phaseConfirmedAt:iso(),phaseConfirmationSource};
  ...
  t={...t,stage:'PAUSING',transitionId,requestedMode:desiredMode,requestedA:desiredA,originalCircuitA:circuitTargetA,startedAt:iso(),stageSince:iso(),failure:null,phaseConfirmedMode:null,phaseConfirmedAt:null,phaseConfirmationSource:null,phaseCloudCheckedAt:null,phaseCloudError:null};
  await pauseSession();
