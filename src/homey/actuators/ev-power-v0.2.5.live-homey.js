// EM v2 | 60 Actuator | EV Power v0.2.5 DEADLINE STALE-EASEE DEGRADE
// Gate-driven. Accept only 0A or 7..16A. Manual Start still normalizes LIVE=false.
// Stale core/Easee state remains visible, but does not fail-close an already validated
// active Pi deadline target. Intent/adapter/gate freshness and all safety contracts remain hard gates.
...
try{
 ...
 const controlFresh=intentAge>=0&&intentAge<=FRESH_MS&&adapterAge>=0&&adapterAge<=FRESH_MS&&gateAge>=0&&gateAge<=FRESH_MS;
 const stateFresh=stateAge>=0&&stateAge<=FRESH_MS;
 ...
 const projection=intent?.policyProjection||{};
 const deadlineAtMs=Date.parse(String(projection.deadlineAt||''));
 const deadlineRemainingKWh=num(projection.deadlineRemainingKWh);
 const deadlineOverrideEligible=projection.deadlineGuardApplied===true&&String(intent?.targets?.ev?.status||'')==='NUMERIC_DEADLINE_TARGET'&&String(intent?.targets?.ev?.source||'')==='REMAINING_KWH_OVER_TIME_TO_DEADLINE'&&targetW>0&&requestedA!==null&&requestedA>=7&&deadlineRemainingKWh!==null&&deadlineRemainingKWh>0&&Number.isFinite(deadlineAtMs)&&deadlineAtMs>Date.now();
 ...
 if(!controlFresh)return await failClosedLive('STALE_INPUT',{intentAgeSec:Math.round(intentAge/1000),adapterAgeSec:Math.round(adapterAge/1000),stateAgeSec:Math.round(stateAge/1000),gateAgeSec:Math.round(gateAge/1000),r,staleClass:'CONTROL'});
 ...
 if(!stateFresh&&!deadlineOverrideEligible)return await failClosedLive('STALE_INPUT',{intentAgeSec:Math.round(intentAge/1000),adapterAgeSec:Math.round(adapterAge/1000),stateAgeSec:Math.round(stateAge/1000),gateAgeSec:Math.round(gateAge/1000),r,staleClass:'STATE',deadlineOverrideEligible:false});
 const degraded=!stateFresh&&deadlineOverrideEligible;
 const w=await writeA(requestedA);
 const normalStatus=w.write?(requestedA===0?'WRITE_ZERO_NORMALIZE':'WRITE_OK'):'NOOP_ALREADY_TARGET';
 const degradedStatus=w.write?'WRITE_OK_DEGRADED_DEADLINE_TELEMETRY':'NOOP_DEGRADED_DEADLINE_TELEMETRY';
 await report(degraded?degradedStatus:normalStatus,{targetW,targetA:requestedA,previousA:w.previousA,revision:r,gateRevision:gateR,gateStatus:'PASS',live:true,physicalWritePerformed:w.write,ownership:requestedA===0?'NORMALIZE_AUTOSTART_TO_ZERO':'APPLY_POWER_INTENT',sessionControl:requestedA>=7?'FLOW_STATE_AWARE_START_RESUME':'NONE',degraded,degradedReason:degraded?'EASEE_STATE_STALE_DEADLINE_OVERRIDE':null,stateAgeSec:Math.round(stateAge/1000),deadlineOverrideEligible});return true;
