// PI Dynamic Planner -> EM2 Power Intent bridge v1.4.0 PHASE-SHADOW.
...
const POLICY='PI_DYNAMIC_PLANNER_BRIDGE_V1.4.0_PHASE_SHADOW';
...
const P1_UP_THRESHOLD_W=EV_W_PER_A, P1_DOWN_THRESHOLD_W=250;
...
let realtime={schema:'EM2_EV_REALTIME_EXECUTION_V0.1',eligible:false,applied:false,fallbackReason:null,envelopeSchema:null,envelopeAllowed:false,minA:0,maxA:0,p1W:null,p1AgeSec:null,chargeState:null,evActualW:null,evPowerAgeSec:null,counterfactualSurplusW:null,candidateA:null,candidateW:null,plannerTargetA:0,plannerTargetW:0,phasePolicy:null,phaseShadow:{schema:'EM2_EV_PHASE_EXECUTION_SHADOW_V0.1',mode:'OFF',requestedA:0,requestedW:0,reason:'NOT_EVALUATED',modeSinceAt:null,shadow:true,deviceWrites:false,controlWrites:false}};
  ...
  if(envValid){
    ...
    try{
      ...
      if(!connectedState(chargeState)){evW=0;evStatus='IDLE';source='HOMEY_BOUNDED_REALTIME_PV';reason='REALTIME_TESLA_NOT_CONNECTED';realtime.applied=true;realtime.fallbackReason=null;realtime.p1W=Math.round(p1W);realtime.candidateA=0;realtime.candidateW=0;}
        ...
        // Easee telemetry is observability only and never gates PV regulation.
        // Controller state comes from our own previous realtime command.
        // Easee telemetry is not required for realtime regulation.
        ...
        if(phasePolicyValid){
          ...
          const actualEvW=currentA*EV_W_PER_A;
          const availableTotalW=Math.max(0,-p1W+actualEvW);
          ...
          let phaseMode=previousMode,phaseReason='HOLD';
          if(previousMode==='3P'){
          ...
          realtime.phaseShadow={
            ...
            actualProductionA:currentA,
            actualProductionPhaseCount:currentA>0?3:0,
        ...
        if(currentA>=MIN_A){
          ...
          // Increase only one amp per control cycle when at least one full
          // 3-phase amp remains exported. Reduce one amp when material
          // grid import is observed. Otherwise hold.
