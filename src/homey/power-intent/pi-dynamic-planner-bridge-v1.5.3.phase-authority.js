// PI Dynamic Planner -> EM2 Power Intent bridge v1.5.3 PHASE-AUTHORITY.
...
const POLICY='PI_DYNAMIC_PLANNER_BRIDGE_V1.5.3_PHASE_AUTHORITY';
  ...
  if(envValid){
    ...
    try{
      ...
      if(!effectiveConnected){evW=0;evStatus='IDLE';source='HOMEY_BOUNDED_REALTIME_PV';reason='REALTIME_TESLA_NOT_CONNECTED';realtime.applied=true;realtime.fallbackReason=null;realtime.p1W=Math.round(p1W);realtime.candidateA=0;realtime.candidateW=0;}
        ...
        // Easee connection state is accepted immediately unless active
        // charging/current/power evidence proves a contradictory stale state.
