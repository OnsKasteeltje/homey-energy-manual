// PI Dynamic Planner -> EM2 Power Intent bridge v1.4.2 PHASE-READBACK-SHADOW.
...
const POLICY='PI_DYNAMIC_PLANNER_BRIDGE_V1.4.2_PHASE_READBACK_SHADOW';
  ...
  if(envValid){
    ...
    try{
      ...
      if(!connectedState(chargeState)){evW=0;evStatus='IDLE';source='HOMEY_BOUNDED_REALTIME_PV';reason='REALTIME_TESLA_NOT_CONNECTED';realtime.applied=true;realtime.fallbackReason=null;realtime.p1W=Math.round(p1W);realtime.candidateA=0;realtime.candidateW=0;}
        ...
        const confirmedPhaseRaw=await readPhaseMode(easee);
        const confirmedPhaseMode=normalizePhaseMode(confirmedPhaseRaw);
        const actualProductionPhaseCount=currentA<=0
          ?0
        ...
        if(phasePolicyValid){
          ...
          const phaseReadbackValid=currentA===0||actualProductionPhaseCount===1||actualProductionPhaseCount===3;
          const actualEvW=phaseReadbackValid
            ?currentA*230*(actualProductionPhaseCount||0)
            :0;
