...
assert.equal(evaluatePriceSource(ez, { now, requiredHorizonEnd }).eligible, true);

{
  const result = selectPriceSourceShadow([pbth, ez], { now, requiredHorizonEnd });
...

{
  ...
  const result = selectPriceSourceShadow([badEz, pbth], { now, requiredHorizonEnd });
...

{
  ...
  const evaluation = evaluatePriceSource(badBasis, { now, requiredHorizonEnd });
...

{
  ...
  const evaluation = evaluatePriceSource(stale, { now, requiredHorizonEnd, maxAgeMinutes: 30 });
  assert.equal(evaluation.eligible, false);
...

{
  ...
  const evaluation = evaluatePriceSource(gap, { now, requiredHorizonEnd });
  assert.equal(evaluation.eligible, false);
