# EM v2 | 60 Adapter | EV Power v0.1 SHADOW
...
```js
// EM v2 | 60 Adapter | EV Power v0.1 SHADOW — hardened deterministic 3p mapper
// Contract: EV_target_W only; fixed 3×230V; floor quantization; fail-closed; NO device writes/network/device reads.
...
const intent=parse(by.EM2_Power_Intent?.value),state=parse(by.EM2_State?.value),prev=parse(by.EM2_EV_Power_Adapter?.value);
...
const maxConfig=num(by['EV Max laadstroom A']?.value),MAX_A=Math.floor(Math.min(16,maxConfig??16));
...
await set('EM2_EV_Power_Adapter',JSON.stringify(out));return out.valid;
```
