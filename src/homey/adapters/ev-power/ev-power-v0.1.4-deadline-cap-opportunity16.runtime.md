# EM v2 | 60 Adapter | EV Power v0.1.4 DEADLINE-CAP OPPORTUNITY16 START7 RUN6
...
- Status: exact live runtime baseline captured after promotion
- Policy change: deadline `maxA` only caps deadline charging. Opportunistic PV / buffer charging may use up to 16 A. Running sessions may step down to 6 A; start minimum remains 7 A.

## Runtime HomeyScript

```js
// EM v2 | 60 Adapter | EV Power v0.1.4 DEADLINE-CAP OPPORTUNITY16 START7 RUN6
// Deadline target obeys EV Max laadstroom A. PV opportunity/buffer may use up to 16A.
...
const r=num(intent?.sourceRevision),sr=num(state?.revision),inputSchema=String(intent?.schema||''),ev=intent?.targets?.ev||{},targetW=num(ev?.target_W),targetStatus=String(ev?.status??''),targetSource=String(ev?.source??'');
const PHASES=3,VOLTAGE_V=230,START_MIN_A=7,RUN_MIN_A=6,FRESH_MS=120000,OPPORTUNITY_MAX_A=16;
const maxConfig=num(maxVar?.value),deadlineCapA=Math.floor(Math.min(16,maxConfig??16));
const deadlineTarget=targetStatus==='NUMERIC_DEADLINE_TARGET'||targetSource==='REMAINING_KWH_OVER_TIME_TO_DEADLINE';
const MAX_A=deadlineTarget?deadlineCapA:OPPORTUNITY_MAX_A;
const maxPolicy=deadlineTarget?'DEADLINE_CAP':'OPPORTUNITY_16A';
...
const chargeState=String(state?.tesla?.chargeState??'unknown'),cs=chargeState.toLowerCase(),teslaCharging=cs==='plugged_in_charging',MIN_A=teslaCharging?RUN_MIN_A:START_MIN_A;
...
if(r!==null&&prev?.schema==='EM2_EV_POWER_ADAPTER_V0.1'&&num(prev?.sourceRevision)===r&&prev?.inputSchema===inputSchema&&num(prev?.input?.target_W)===targetW&&num(prev?.electrical?.max_A)===MAX_A&&String(prev?.input?.chargeState??'')===chargeState&&String(prev?.electrical?.max_policy??'')===maxPolicy&&prev?.safety?.mappingRevision==='FLOOR_3P230_START7_RUN6_FAIL_CLOSED')return true;
