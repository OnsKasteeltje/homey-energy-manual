// PREP TEMPLATE ONLY — DO NOT DEPLOY UNTIL Logic IDs ARE RESOLVED FROM HOMEY.
// EM v2 | 60 Adapter | EV Power v0.2 SHADOW LOW-LOAD
// Derived strictly from captured v0.1 runtime semantics.
// Change scope: replace broad getVariables() with targeted getVariable() reads and targeted output update.

const ID = {
  powerIntent: '__RESOLVE_EM2_POWER_INTENT_ID__',
  state: '__RESOLVE_EM2_STATE_ID__',
  previousAdapter: '__RESOLVE_EM2_EV_POWER_ADAPTER_ID__',
  maxChargeA: '__RESOLVE_EV_MAX_LAADSTROOM_A_ID__',
  outputAdapter: '__RESOLVE_EM2_EV_POWER_ADAPTER_ID__'
};

const [intentV,stateV,prevV,maxV] = await Promise.all([
  Homey.logic.getVariable({id:ID.powerIntent}),
  Homey.logic.getVariable({id:ID.state}),
  Homey.logic.getVariable({id:ID.previousAdapter}),
  Homey.logic.getVariable({id:ID.maxChargeA})
]);
...
const intent=parse(intentV?.value),state=parse(stateV?.value),prev=parse(prevV?.value);
...
const maxConfig=num(maxV?.value),MAX_A=Math.floor(Math.min(16,maxConfig??16));
...
if(r!==null&&prev?.schema==='EM2_EV_POWER_ADAPTER_V0.1'&&num(prev?.sourceRevision)===r&&prev?.inputSchema===inputSchema&&num(prev?.input?.target_W)===targetW&&num(prev?.electrical?.max_A)===MAX_A&&prev?.safety?.mappingRevision==='FLOOR_3P230_FAIL_CLOSED') return true;
